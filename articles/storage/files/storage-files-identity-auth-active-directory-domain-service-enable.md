...
1.  **Schakel Azure AD Domain Services in op de Azure AD-tenant.**
    ...
    Als u verificatie met Azure AD-referenties wilt ondersteunen, moet u Azure AD Domain Services voor uw Azure AD-tenant. Als u niet de beheerder van de Azure AD-tenant bent, neem dan contact op met de beheerder en volg de stapsgewijs richtlijnen voor het inschakelen van Azure Active Directory Domain Services met behulp van [Azure Portal](../../active-directory-domain-services/tutorial-create-instance.md).

    Het duurt doorgaans ongeveer 15 minuten voordat een Azure AD DS voltooid. Controleer of de status van Azure AD DS **wordt** uitgevoerd, met wachtwoordhashsynchronisatie ingeschakeld, voordat u doorgaat met de volgende stap.
...
1.  **Domein toevoegen aan een Azure-VM met Azure AD DS.**
    ...
    > [!NOTE]
    > Azure AD DS verificatie via SMB met Azure-bestands shares wordt alleen ondersteund op Azure-VM's met besturingssysteemversies boven Windows 7 of Windows Server 2008 R2.
...
1.  **Selecteer of maak een Azure-bestands share.**
    ...
    Voor optimale prestaties wordt u aangeraden dat uw bestands share zich in dezelfde regio als de VM van waaruit u toegang tot de share wilt krijgen.

1.  **Controleer Azure Files verbinding kunt maken door Azure-bestandssleutels te verbinden met behulp van uw opslagaccountsleutel.**
...
Voordat u verificatie Azure AD DS via SMB voor Azure-bestands shares inschakelen, controleert u of uw Azure AD- en Azure Storage goed zijn geconfigureerd. U wordt aangeraden de vereisten [te doorlopen om](#prerequisites) ervoor te zorgen dat u alle vereiste stappen hebt voltooid.
...
4. Een Azure-bestands share van een domein-VM te maken.
...
Als u Azure AD DS via SMB voor Azure Files wilt inschakelen, kunt u een eigenschap voor opslagaccounts instellen met behulp van de Azure Portal, Azure PowerShell of Azure CLI. Deze eigenschap impliciet instellen 'domein wordt gekoppeld' aan het opslagaccount met de gekoppelde Azure AD DS implementatie. Azure AD DS via SMB wordt ingeschakeld voor alle nieuwe en bestaande bestands shares in het opslagaccount.
...
Als u Azure AD DS via SMB wilt inschakelen met Azure PowerShell, installeert u de nieuwste Az-module (2.4 of nieuwer) of de Az.Storage-module (1.5 of nieuwer). Zie Install Azure PowerShell on Windows with PowerShellGet (Windows installeren met [PowerShellGet) voor](/powershell/azure/install-Az-ps)meer informatie over het installeren van PowerShell.

Als u een nieuw opslagaccount wilt maken, roept u [New-AzStorageAccount](/powershell/module/az.storage/New-azStorageAccount)aan en stelt u vervolgens de parameter **EnableAzureActiveDirectoryDomainServicesForFile** in op **true**. In het volgende voorbeeld moet u de waarden van de tijdelijke aanduidingen vervangen door uw eigen waarden. (Als u de vorige preview-module gebruikt, is de parameter voor het inschakelen van de functie **EnableAzureFilesAadIntegrationForSMB**.)
...
Als u een nieuw opslagaccount wilt maken, roept u [az storage account create aan](/cli/azure/storage/account#az_storage_account_create)en stelt u de eigenschap in op `--enable-files-aadds` **true**. In het volgende voorbeeld moet u de waarden van de tijdelijke aanduidingen vervangen door uw eigen waarden. (Als u de vorige preview-module hebt gebruikt, is de parameter voor het inschakelen van functies **file-aad**.)
...
U hebt nu verificatie via Azure AD DS via SMB ingeschakeld en een aangepaste rol toegewezen die toegang biedt tot een Azure-bestands share met een Azure AD-identiteit. Volg de instructies in de [secties Toegangsmachtigingen](#assign-access-permissions-to-an-identity) toewijzen voor het gebruik van een identiteit en NTFS-machtigingen configureren via SMB om extra gebruikers toegang te verlenen tot [uw bestands share.](#configure-ntfs-permissions-over-smb)

## <a name="next-steps"></a>Volgende stappen

Zie de volgende resources voor Azure Files informatie over het gebruik van Azure AD via SMB:
