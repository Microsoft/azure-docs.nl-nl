...
| end_session_endpoint | Ja | De URL van het eind eindpunt van de sessie conform RFC 6749. |
...
| BearerTokenTransmissionMethod | Nee | Hiermee geeft u op hoe het token wordt verzonden. De standaard methode is een query reeks. Als u het token wilt verzenden als een aanvraag header, stelt u in op `AuthorizationHeader` . |
