...
### <a name="can-i-configure-the-nfs-export-policy-rules-to-control-access-to-the-azure-netapp-files-service-mount-target"></a>Kan ik de regels voor het NFS-exportbeleid configureren om de toegang tot het Azure NetApp Files van de service te controleren?

Ja, u kunt maximaal vijf regels configureren in één NFS-exportbeleid.

### <a name="does-azure-netapp-files-support-network-security-groups"></a>Biedt Azure NetApp Files ondersteuning voor netwerkbeveiligingsgroepen?

Nee, momenteel kunt u netwerkbeveiligingsgroepen niet toepassen op het gedelegeerde subnet van Azure NetApp Files of de netwerkinterfaces die door de service zijn gemaakt.
...
Zie Bewerkingen van Azure-resourceproviders Azure NetApp Files voor de volledige lijst met [`Microsoft.NetApp`](../role-based-access-control/resource-provider-operations.md#microsoftnetapp) machtigingen.
...
### <a name="what-should-i-do-to-optimize-or-tune-azure-netapp-files-performance"></a>Wat moet ik doen om de prestaties van Azure NetApp Files optimaliseren?

U kunt de volgende acties uitvoeren volgens de prestatievereisten: 
- Zorg ervoor dat de grootte van de virtuele machine juist is aangepast.
...
Azure NetApp Files biedt metrische gegevens over volumeprestaties. U kunt ook een Azure Monitor voor het bewaken van metrische gegevens over gebruik voor Azure NetApp Files.  Zie [Metrische gegevens Azure NetApp Files](azure-netapp-files-metrics.md) voor de lijst met metrische prestatiegegevens voor Azure NetApp Files.

### <a name="whats-the-performance-impact-of-kerberos-on-nfsv41"></a>Wat is de invloed op de prestaties van Kerberos op NFSv4.1?

Zie Impact van de prestaties van [Kerberos op NFSv4.1-volumes](performance-impact-kerberos.md) voor informatie over beveiligingsopties voor NFSv4.1, de geteste prestatievectoren en de verwachte invloed op de prestaties. 

## <a name="nfs-faqs"></a>Veelgestelde vragen over NFS

### <a name="i-want-to-have-a-volume-mounted-automatically-when-an-azure-vm-is-started-or-rebooted--how-do-i-configure-my-host-for-persistent-nfs-volumes"></a>Ik wil een volume automatisch laten monteren wanneer een Azure-VM wordt gestart of opnieuw wordt opgestart.  Hoe kan ik mijn host configureren voor permanente NFS-volumes?

Als u wilt dat een NFS-volume automatisch wordt bevestigd bij het starten of opnieuw opstarten van de VM, voegt u een vermelding toe aan het `/etc/fstab` bestand op de host. 
...
De volumegrootte die in DF wordt vermeld, is de maximale grootte Azure NetApp Files het volume kan groeien. De grootte van het Azure NetApp Files volume in DF opdracht is niet weerspiegelend van het quotum of de grootte van het volume.  U kunt de volumegrootte Azure NetApp Files quotum via de Azure Portal of de API.

### <a name="what-nfs-version-does-azure-netapp-files-support"></a>Welke NFS-versie wordt Azure NetApp Files ondersteund?

Azure NetApp Files ondersteunt NFSv3 en NFSv4.1. U kunt [een volume maken met](azure-netapp-files-create-volumes.md) behulp van een van beide NFS-versies. 
...
U maakt bijvoorbeeld een volume met de naam `vol1` . Vervolgens maakt u een ander volume met de naam `vol1` in een andere capaciteitspool, maar in hetzelfde abonnement en dezelfde regio. In dit geval veroorzaakt het gebruik van dezelfde volumenaam `vol1` een fout. Als u hetzelfde bestandspad wilt gebruiken, moet de naam zich in een andere regio of een ander abonnement.

### <a name="when-i-try-to-access-nfs-volumes-through-a-windows-client-why-does-the-client-take-a-long-time-to-search-folders-and-subfolders"></a>Wanneer ik toegang probeer te krijgen tot NFS-volumes via een Windows-client, waarom duurt het dan lang voordat de client mappen en submappen zoekt?
...
Voor NFSv4.1-clients ondersteunt Azure NetApp Files het NFSv4.1-bestandsvergrendelingsmechanisme dat de status van alle bestandsvergrendelingen onder een leasemodel behoudt. 

Volgens RFC 3530 definieert Azure NetApp Files één leaseperiode voor alle statussen die worden vastgehouden door een NFS-client. Als de client de lease niet binnen de gedefinieerde periode vernieuwt, worden alle staten die zijn gekoppeld aan de lease van de client vrijgegeven door de server.  

Als een client die een volume mounting bijvoorbeeld niet meer reageert of na time-outs vast loopt, worden de vergrendelingen vrijgegeven. De client kan de lease expliciet of impliciet vernieuwen door bewerkingen uit te voeren zoals het lezen van een bestand.   
...
Azure NetApp Files ondersteunt SMB 2.1 en SMB 3.1 (dat ondersteuning biedt voor SMB 3.0).    
...
### <a name="does-azure-netapp-files-support-azure-active-directory"></a>Biedt Azure NetApp Files ondersteuning voor Azure Active Directory? 

Zowel [Azure Active Directory (AD) Domain Services](../active-directory-domain-services/overview.md) als Active Directory Domain Services [(AD DS)](/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview) worden ondersteund. U kunt bestaande Active Directory-domeincontrollers gebruiken met Azure NetApp Files. Domeincontrollers kunnen zich in Azure bevinden als virtuele machines of on-premises via ExpressRoute of S2S VPN. Azure NetApp Files biedt op dit moment geen ondersteuning voor [AD-Azure Active Directory.](https://azure.microsoft.com/resources/videos/azure-active-directory-overview/)

Als u een Azure NetApp Files met Azure Active Directory Domain Services, is het pad naar de organisatie-eenheid wanneer u Active Directory configureert `OU=AADDC Computers` voor uw NetApp-account.
...
Stel als best practice maximumtolerantie voor synchronisatie van computerklok in op vijf minuten. Zie voor meer informatie Maximale [tolerantie voor synchronisatie van computerklok.](/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/jj852172(v=ws.11)) 

### <a name="can-i-manage-smb-shares-sessions-and-open-files-through-computer-management-console-mmc"></a>Kan ik `SMB Shares` , en beheren via Computer Management Console `Sessions` `Open Files` (MMC)?
...
Gebruik de **koppeling JSON-weergave** in het volumeoverzichtsvenster en zoek naar de **id van het startIp** **onder**  ->  **eigenschappen mountTargets**.

### <a name="can-an-azure-netapp-files-smb-share-act-as-an-dfs-namespace-dfs-n-root"></a>Kan een Azure NetApp Files SMB-share fungeren als DFS-naamruimte (DFS-N)-hoofdmap?

Nee. SMB Azure NetApp Files-shares kunnen echter fungeren als een DFS-naamruimte (DFS-N)-mapdoel.   
Als u een Azure NetApp Files SMB-share wilt gebruiken als een DFS-N-mapdoel, geeft u het unc-pad (Universal Naming Convention) van de Azure NetApp Files SMB-share op met behulp van de procedure [Mapdoel DFS](/windows-server/storage/dfs-namespaces/add-folder-targets#to-add-a-folder-target) toevoegen.  
...
Bij het verzenden van een aanvraag naar de opslag versleutelt de client de aanvraag, die vervolgens door de opslag wordt ontsleuteld. Antwoorden worden op dezelfde manier versleuteld door de server en ontsleuteld door de client.
...
#### <a name="with-azure-netapp-files-at-what-layer-is-smb-encryption-enabled"></a>Met Azure NetApp Files, op welke laag is SMB-versleuteling ingeschakeld?  
...
Hoewel SMB-versleuteling van invloed is op zowel de client (CPU-overhead voor het versleutelen en ontsleutelen van berichten) als de opslag (afname van de doorvoer), worden in de volgende tabel alleen de opslagimpact belicht. U moet de impact van de versleutelingsprestaties op uw eigen toepassingen testen voordat u workloads in productie implementeert.

|     I/O-profiel       |     Impact        |
|-  |-  |
|     Workloads lezen en schrijven      |     10% tot 15%        |
|     Metagegevens intensief        |     5%    |

## <a name="capacity-management-faqs"></a>Veelgestelde vragen over capaciteitsbeheer

### <a name="how-do-i-monitor-usage-for-capacity-pool-and-volume-of-azure-netapp-files"></a>Hoe kan ik gebruik bewaken voor capaciteitspool en volume van Azure NetApp Files? 
...
Azure NetApp Files biedt NFS- en SMB-volumes.  U kunt elk hulpprogramma voor het kopiëren van bestanden gebruiken om gegevens naar de service te migreren. 
...
- Valideer de netwerkconnectiviteit tussen de bron en Azure NetApp Files IP-adres van het doelvolume. Gegevensoverdracht tussen on-premises en de Azure NetApp Files-service wordt ondersteund via ExpressRoute.
...
U kunt Azure NetApp Files NFS-volumes aan AVS-VM's met Windows of Linux-VM's. U kunt de Azure NetApp Files SMB-shares op AVS Windows-VM's. Zie voor meer informatie [Azure NetApp Files met Azure VMware Solution.]( ../azure-vmware/netapp-files-with-azure-vmware-solution.md)  

### <a name="what-regions-are-supported-for-using-azure-netapp-files-nfs-or-smb-volumes-with-azure-vmware-solution-avs"></a>Welke regio's worden ondersteund voor het gebruik Azure NetApp Files NFS- of SMB-volumes met Azure VMware Solution (AVS)?
