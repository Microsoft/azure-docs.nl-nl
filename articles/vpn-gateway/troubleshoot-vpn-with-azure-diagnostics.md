...
U ziet dat er meerdere kolommen beschikbaar zijn in deze tabellen. In dit artikel presenteert u alleen de meest relevante voor waarden voor eenvoudiger logboek gebruik.
...
Wijzigingen in de configuratie worden gecontroleerd in de tabel **GatewayDiagnosticLog** . Houd er rekening mee dat het enkele minuten kan duren voordat wijzigingen die u uitvoert, worden weer gegeven in de logboeken.
...
|**OperationName** |de gebeurtenis die heeft plaatsgevonden. Dit kan een van *SetGatewayConfiguration, SetConnectionConfiguration, HostMaintenanceEvent, GatewayTenantPrimaryChanged, MigrateCustomerSubscription, GatewayResourceMove, ValidateGatewayConfiguration*|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-26-set-gateway.png" alt-text="Voor beeld van een gateway-bewerking instellen.":::
...
| **Exemplaar \_ s** | de instantie van de gateway die de gebeurtenis heeft geactiveerd. Dit kan een GatewayTenantWorker zijn \_ in \_ 0 of GatewayTenantWorker \_ in \_ 1. Dit zijn de namen van de twee exemplaren van de gateway.|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-16-tunnel-connected.png" alt-text="Voor beeld van een verbonden tunnel gebeurtenis.":::
...
|---        | ---               |
|**TimeGenerated** | de tijds tempel van elke gebeurtenis, in UTC-tijd zone|
|**OperationName** | de gebeurtenis die heeft plaatsgevonden. Dit kan een van *StaticRouteUpdate, BgpRouteUpdate, BgpConnectedEvent, BgpDisconnectedEvent*|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-31-bgp-route.png" alt-text="Voor beelden van BGP-routes.":::
...
|---        | ---               |
|**TimeGenerated** | de tijds tempel van elke gebeurtenis, in UTC-tijd zone|
| **RemoteIP** | het IP-adres van het on-premises VPN-apparaat. In praktijk scenario's is het nuttig om te filteren op het IP-adres van het relevante on-premises apparaat. er zijn er meer dan één |
...
|**Gebeurtenis** | bevat een diagnostisch bericht dat nuttig is voor het oplossen van problemen. Ze beginnen meestal met een tref woord en verwijzen naar de acties die worden uitgevoerd door de Azure gateway **\[ Send \]** geeft aan dat een gebeurtenis die is veroorzaakt door een IPSec-pakket dat is verzonden door de Azure-gateway die wordt **\[ ontvangen \]** , een gebeurtenis aanduidt als gevolg van een pakket dat is ontvangen van on-premises **\[ lokaal \]** apparaat geeft een actie aan die lokaal door de Azure-gateway wordt uitgevoerd |
...
- Als de IPsec-tunnel niet tot stand kan worden gebracht, zal Azure elke paar seconden blijven proberen. Daarom is het oplossen van problemen met VPN-verbindingen zeer handig voor IKEdiagnosticLog omdat u niet hoeft te wachten op een bepaalde tijd om het probleem te reproduceren. Daarnaast is de fout in theorie altijd hetzelfde als we proberen, zodat u op elk gewenst moment net kunt inzoomen op één ' voor beeld ' van een mislukte onderhandeling.
...
De laatst beschik bare tabel voor de diagnostische gegevens van de VPN-verbinding is **P2SDiagnosticLog**. Hiermee wordt de activiteit getraceerd voor Point-to-site.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-28-p2s-log-event.png" alt-text="Voor beeld van P2S-Logboeken.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-29-eap.png" alt-text="Voor beeld van EAP-Logboeken.":::
