...
1. Uw invoer bericht opmaken met een XML-envelop. Zie voor een voor beeld de volgende XML-nettolading:
...
##### <a name="the-requestcontext-on-the-ireplychannel-was-closed-without-a-reply-being-sent"></a>De RequestContext op de IReplyChannel is gesloten zonder dat er een antwoord wordt verzonden

Dit fout bericht betekent dat er onverwachte fouten optreden wanneer de catch-all-handler voor het kanaal het kanaal beëindigt als gevolg van een fout en het kanaal opnieuw opbouwt om andere berichten te verwerken.

Als u wilt bevestigen dat uw logische app de IDoc heeft ontvangen, [voegt u een reactie actie toe](../connectors/connectors-native-reqres.md#add-a-response-action) die een `200 OK` status code retourneert. Laat de hoofd tekst leeg en wijzig of voeg deze niet toe aan de kopteksten. De IDoc wordt getransporteerd via tRFC, die geen respons lading toestaat.

Als u in plaats daarvan de IDoc wilt afwijzen, moet u reageren met een andere HTTP-status code dan `200 OK` . De SAP-adapter retourneert vervolgens uit uw naam een uitzonde ring in SAP. U moet de IDoc alleen afwijzen om Trans Port-fouten te Signa leren op SAP, zoals een niet-gerouteerde IDoc die uw toepassing niet kan verwerken. U moet geen IDoc afwijzen voor fouten op toepassings niveau, zoals problemen met de gegevens die zijn opgenomen in de IDoc. Als u het accepteren van het Trans Port vertraagt voor validatie op toepassings niveau, kunnen er negatieve prestaties optreden vanwege het blok keren van uw verbinding van het transporteren van andere IDocs.

Als u dit fout bericht ontvangt en er systeem fouten optreden bij het aanroepen van Logic Apps, controleert u of u de netwerk instellingen voor uw on-premises gegevens Gateway Service hebt geconfigureerd voor uw specifieke omgeving. Als uw netwerk omgeving bijvoorbeeld het gebruik van een proxy voor het aanroepen van Azure-eind punten vereist, moet u uw on-premises gegevens Gateway Service configureren voor het gebruik van uw proxy. Zie [Proxy Configuration](/dotnet/framework/network-programming/proxy-configuration)(Engelstalig) voor meer informatie.

Als u dit fout bericht ontvangt en problemen ondervindt met het aanroepen van Logic Apps, moet u mogelijk het aantal nieuwe pogingen en/of het interval voor nieuwe pogingen verhogen. 

1. Controleer de SAP-instellingen in het configuratie bestand van uw on-premises gegevens Gateway Service `Microsoft.PowerBI.EnterpriseGateway.exe.config` . De instelling aantal nieuwe pogingen ziet er als volgt uit `WebhookRetryMaximumCount="2"` . De instelling voor het interval voor opnieuw proberen ziet eruit als `WebhookRetryDefaultDelay="00:00:00.10"` de time span-indeling `HH:mm:ss.ff` . 
    
1. Sla uw wijzigingen op en start de on-premises gegevens Gateway opnieuw op.

##### <a name="the-segment-or-group-definition-e2edk36001-was-not-found-in-the-idoc-meta"></a>Het segment of de groeps definitie E2EDK36001 is niet gevonden in de meta IDoc

Dit fout bericht geeft aan dat de verwachte fouten worden veroorzaakt door andere fouten. Bijvoorbeeld, de fout bij het genereren van een IDoc XML-nettolading omdat de bijbehorende segmenten niet worden vrijgegeven door SAP. Als gevolg hiervan ontbreken de meta gegevens van het segment type dat vereist is voor de conversie.
