---
title: Een CopyVideo CopyAudio-trans formatie maken
description: Maak een CopyVideo CopyAudio-trans formatie met behulp van Media Services-API.
services: media-services
documentationcenter: ''
author: IngridAtMicrosoft
manager: femila
editor: ''
ms.assetid: ''
ms.service: media-services
ms.devlang: multiple
ms.topic: how-to
ms.tgt_pltfrm: multiple
ms.workload: na
ms.date: 11/19/2020
ms.author: inhenkel
ms.openlocfilehash: 9a3d9ff9947a97182964f805d0b8da7e3abcad69
ms.sourcegitcommit: 02bc06155692213ef031f049f5dcf4c418e9f509
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 04/03/2021
ms.locfileid: "106281913"
---
# <a name="create-a-copyvideo-copyaudio-transform"></a>Een CopyVideo CopyAudio-trans formatie maken

[!INCLUDE [media services api v3 logo](./includes/v3-hr.md)]

In dit artikel wordt beschreven hoe u een `CopyVideo/CopyAudio` trans formatie maakt.

## <a name="prerequisites"></a>Vereisten

Volg de stappen in [Een Media Services-account maken](./account-create-how-to.md) om het vereiste Media Services-account en de resourcegroep te maken om een asset te maken.

## <a name="methods"></a>Methoden

### <a name="using-the-rest-api"></a>De REST API gebruiken

[!INCLUDE [task-create-copy-video-audio-rest.md](./includes/task-create-copy-video-audio-rest.md)]

## <a name="next-steps"></a>Volgende stappen

[!INCLUDE [transforms next steps](./includes/transforms-next-steps.md)]
