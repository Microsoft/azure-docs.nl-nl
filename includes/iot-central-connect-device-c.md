...
ms.locfileid: "97033880"
---
## <a name="prerequisites"></a>Vereisten

Om de stappen in dit artikel te kunnen uitvoeren, hebt u een Azure IoT Central-toepassing nodig die is gemaakt met behulp van de sjabloon **Aangepaste toepassing** . Zie voor meer informatie de [snelstart over het maken van een toepassing](../articles/iot-central/core/quick-deploy-iot-central.md). De toepassing moet op of na 14 juli 2020 zijn gemaakt.
...
Open in de instantie van de Microsoft Azure IoT-SDK voor C die u eerder hebt gedownload, het bestand *azure-iot-sdk-c/iothub_client/samples/pnp/pnp_simple_thermostat/pnp_simple_thermostat.c* in een teksteditor.
...
De functie `main` roept eerst `CreateAndConfigureDeviceClientHandleForPnP` aan om het volgende te doen:
...
* Een handler voor het bijwerken van eigenschappen voor de schrijfbare eigenschap `targetTemperature` te maken.
...
De functie `Thermostat_SendCurrentTemperature` laat zien hoe het apparaat de temperatuurtelemetriegegevens verzendt naar IoT Central:
...
De functie `SendMaxTemperatureSinceReboot` verzendt een `maxTempSinceLastReboot`-eigenschap-update naar IoT Central:
...
De functie `Thermostat_DeviceTwinCallback` verwerkt schrijfbare eigenschapupdates van IoT Central:
...
De functie `Thermostat_DeviceMethodCallback` handelt opdrachten af die uit IoT Central zijn aangeroepen:
