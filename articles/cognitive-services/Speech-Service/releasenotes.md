...
- **Java script**: eenvoudigere installatie voor spraak assistenten en botsingen [`BotFrameworkConfig`](https://docs.microsoft.com/javascript/api/microsoft-cognitiveservices-speech-sdk/botframeworkconfig) , die nu `fromHost()` en `fromEndpoint()` Factory-methoden hebben waarmee het gebruik van aangepaste service locaties en hand matig ingestelde eigenschappen wordt vereenvoudigd. We hebben ook een standaard optionele specificatie van `botId` voor het gebruik van een niet-standaard bot over de configuratie fabrieken.
