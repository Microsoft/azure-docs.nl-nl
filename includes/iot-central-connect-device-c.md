...
ms.locfileid: "106491077"
---
## <a name="prerequisites"></a>Vereisten

Als u de stappen in dit artikel wilt uitvoeren, moet u een Azure IoT Central-toepassing maken met behulp van de sjabloon voor **aangepaste toepassingen** . Zie voor meer informatie de [snelstart over het maken van een toepassing](../articles/iot-central/core/quick-deploy-iot-central.md). De toepassing moet op of na 14 juli 2020 zijn gemaakt.
...
Open in de kopie van de Microsoft Azure IoT SDK voor C die u eerder hebt gedownload de *Azure-IOT-SDK-C/iothub_client/samples/pnp/pnp_temperature_controller/pnp_temperature_controller. C* en *Azure-IOT-SDK-C/iothub_client/samples/pnp/pnp_temperature_controller/pnp_thermostat_component. C* -bestanden in een tekst editor.
...
In *pnp_temperature_controller. c* wordt de `main` functie eerst aangeroepen `CreateDeviceClientAndAllocateComponents` voor het volgende:

* De model-id `dtmi:com:example:Thermostat;1` in te stellen. IoT Central gebruikt de model-id om de apparaatsjabloon voor dit apparaat te identificeren of te genereren. Zie [Een apparaat koppelen met een apparaatsjabloon](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template) voor meer informatie.
...
* Hiermee maakt u een handler voor opdrachten in het onderdeel temperatuur controller.
* Hiermee maakt u een handler voor eigenschaps updates in het onderdeel temperatuur controller.
* Maakt de twee Thermo staat-onderdelen.

De `main` functie volgende:

* Rapporteert enkele initiële eigenschaps waarden voor alle onderdelen.
* Hiermee wordt een lus gestart voor het verzenden van telemetrie van alle onderdelen.
...
In `pnp_thermostat_component.c` geeft de `PnP_ThermostatComponent_SendTelemetry` functie aan hoe het apparaat de telemetrie van de Tempe ratuur verzendt van een onderdeel naar IOT Central:
...
In `pnp_thermostat_component.c` verzendt de `PnP_TempControlComponent_Report_MaxTempSinceLastReboot_Property` functie een `maxTempSinceLastReboot` eigenschaps update van het onderdeel naar IOT Central:
...
In `pnp_thermostat_component.c` verwerkt de `PnP_ThermostatComponent_ProcessPropertyUpdate` functie Beschrijf bare eigenschaps updates van IOT Central:
...
In `pnp_thermostat_component.c` wordt met de `PnP_ThermostatComponent_ProcessCommand` functie opdrachten verwerkt die worden aangeroepen vanuit IOT Central:
