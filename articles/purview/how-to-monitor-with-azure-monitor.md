...
2. Registreer de gebeurtenissen in een opslag account. Een specifiek opslag account wordt aanbevolen voor het archiveren van de diagnostische Logboeken. Volg dit artikel om [een opslag account te maken](../storage/common/storage-quickstart-create-account.md?tabs=azure-portal).
...
Het duurt Maxi maal 15 minuten om logboeken te ontvangen in het zojuist gemaakte opslag account. [Zie gegevens retentie en schema van bron Logboeken in azure Storage-account](../azure-monitor/platform/resource-logs-collect-storage.md). Zodra de diagnostische logboeken zijn geconfigureerd, stroomt de gebeurtenissen naar het opslag account.
