...
-  Er zijn zes nieuwe talen geïntroduceerd: 12 nieuwe stemmen in 6 nieuwe landinstellingen worden toegevoegd aan de neurale TTS-taallijst: Nia in `cy-GB` Moet (Verenigd Koninkrijk), Aled in `cy-GB` Verenigde Koninkrijk, `en-PH` Engelstalige (Engelstalige talen), James in Engels (Engelstalig), Charline in het Frans (Eues), Moet in het Frans (Zij), Dena in het Nederlands (Zij), Arnaud in het Nederlands `en-PH` `fr-BE` `fr-BE` `nl-BE` `nl-BE` (Lands), Polina in `uk-UA` Deens (Zij), Ostap in `uk-UA` Dena (Zij), Uzma in `ur-PK` Urdu (Grens), Asad in `ur-PK` Urdu (China).

- Vijf talen van preview tot **ga** : 10 stemmen in 5 landtalen die in 2020-november zijn geïntroduceerd, zijn nu GA: Kert in `et-EE` Hetens (Engelstalig), Colm in `ga-IE` Eues (Ierland), Nils in `lv-LV` Lets (Lets), Moetas in Deens `lt-LT` (Rusland), Joseph in `mt-MT` Now (Laten) .

- **Nieuwe mannenstem toegevoegd voor Frans (Canada)** - Er is een nieuwe stem beschikbaar voor `fr-CA` Frans (Canada).

- **Kwaliteitsverbetering:** vermindering van het aantal uitspraakfouten in `hu-HU` Hongaars - 48,17%, `nb-NO` Noors - 52,76%, `nl-NL` Nederlands (Nederland) - 22,11%.
...
**Gebeurtenissen van gezichtshoudingen om tekens te animaten**
...
- **Java:** Ondersteuning toegevoegd voor SPEAKER Recognition-API's. Details [hier.](https://docs.microsoft.com/java/api/com.microsoft.cognitiveservices.speech.speakerrecognizer?view=azure-java-stable)
...
- **C++/C#/Java:** ondersteuning toegevoegd voor het ophalen van spraakprofiel voor het scenario voor sprekerherkenning. Details voor [C++](https://docs.microsoft.com/cpp/cognitive-services/speech/speakerrecognizer), [C#](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speakerrecognizer?view=azure-dotnet)en [Java](https://docs.microsoft.com/java/api/com.microsoft.cognitiveservices.speech.speakerrecognizer?view=azure-java-stable).
- **C++/C#/Java/Objective-C/Python:** Ondersteuning toegevoegd voor afzonderlijke gedeelde bibliotheek voor audiomicrofoon en sprekerbesturing. Hierdoor kan de SDK worden gebruikt in omgevingen die geen vereiste audiobibliotheekafhankelijkheden hebben.
- **Objective-C/Swift:** Ondersteuning toegevoegd voor module-framework met umbrella-header. Hiermee kunt u de Speech-SDK importeren als een module in iOS/Mac Objective-C/Swift-apps. Hiermee wordt het [GitHub-probleem #452.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues/452)
- **Python:** ondersteuning toegevoegd voor [Python 3.9](https://docs.microsoft.com/azure/cognitive-services/speech-service/quickstarts/setup-platform?pivots=programming-language-python) en geen ondersteuning meer voor Python 3.5 per Levensduur van Python voor [3.5](https://devguide.python.org/devcycle/#end-of-life-branches).
...
- Verbeterde nauwkeurigheid, leesbaarheid en zie ook secties van onze C#-referentiedocumentatie [hier](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech?view=azure-dotnet).
...
- **JavaScript:** het probleem met de timing van de microfoon is opgelost als de microfoonstream eindigt vóór het stoppen van de herkenning, om een probleem op te lossen met spraakherkenning dat niet werkt in Firefox.
- **JavaScript:** de initialisatie-promise wordt nu correct verwerkt wanneer de browser microfoon afdeert voordat inschakelen is voltooid.
- **JavaScript:** de URL-afhankelijkheid is vervangen door url-parse. Hiermee wordt het [GitHub-probleem #264.](https://github.com/microsoft/cognitive-services-speech-sdk-js/issues/264)
- **Android:** vaste callbacks werken niet wanneer `minifyEnabled` is ingesteld op true.
- **C++/C#/Java/Objective-C/Python:** wordt correct ingesteld op de onderliggende `TCP_NODELAY` socket-IO voor TTS om de latentie te verminderen.
...
- **C++/C#/Java:** Er is een incidentele crash in de vernietigende speaker recognizer opgelost.
...
- Recognize en intent kunnen nu Azure-functies gebruiken om het aantal woordenfouten te berekenen met behulp van `spx recognize --wer url <URL>` .
...
- Kleiner geheugen en schijfvoetafdruk maken de SDK efficiënter.
- Uitvoerindelingen met een hogere betrouwbaarheid zijn beschikbaar voor de persoonlijke preview van aangepaste neurale spraak.
- Intent Recognizer kan nu meer dan de bovenste intentie retourneren, zodat u een afzonderlijke evaluatie kunt maken over de intentie van uw klant.
...
- Verbeterde prestaties van het apparaat door compressie optioneel te maken.
...
- Er zijn verschillende oplossingen voor problemen die u, onze waardevolle klanten, hebben gemarkeerd op GitHub. Dank u! Blijf op de hoogte van de feedback.
...
**Nieuwe functies**
- **Alle:** nieuwe 48 KHz-uitvoerindelingen beschikbaar voor de privépreview van aangepaste neurale spraak via de TTS-spraaksynthese-API: Audio48Khz192KBitRateMonoMp3, audio-48 khz-192kbitrate-mono-mp3, Audio48Khz96KBitRateMonoMp3, audio-48khz-96kbitrate-mono-mp3, Raw48Khz16BitMonoPcm, raw-48khz-16bit-mono-pcm,Streamf48Khz16BitMonoPcm, rawf-48khz-16bit-mono-pcm.
- **Alle:** Aangepaste stem is ook eenvoudiger te gebruiken. Ondersteuning toegevoegd voor het instellen van custom voice via `EndpointId` ([C++](/cpp/cognitive-services/speech/speechconfig#setendpointid), [C#](/dotnet/api/microsoft.cognitiveservices.speech.speechconfig.endpointid#Microsoft_CognitiveServices_Speech_SpeechConfig_EndpointId), [Java](/java/api/com.microsoft.cognitiveservices.speech.speechconfig.setendpointid#com_microsoft_cognitiveservices_speech_SpeechConfig_setEndpointId_String_), [JavaScript](/javascript/api/microsoft-cognitiveservices-speech-sdk/speechconfig#endpointId), [Objective-C](/objectivec/cognitive-services/speech/spxspeechconfiguration#endpointid), [Python](/python/api/azure-cognitiveservices-speech/azure.cognitiveservices.speech.speechconfig#endpoint-id)). Vóór deze wijziging moesten custom voice-gebruikers de eindpunt-URL instellen via de `FromEndpoint` methode . Klanten kunnen nu de methode `FromSubscription` gebruiken, net als openbare stemmen, en vervolgens de implementatie-id verstrekken door in te `EndpointId` stellen. Dit vereenvoudigt het instellen van aangepaste stemmen. 
- **C++/C#/Java/Objective-C/Python:** haal meer op dan de belangrijkste intentie van `IntentRecognizer` . Het biedt nu ondersteuning voor het configureren van het JSON-resultaat met alle intenties en niet alleen de best scorende intentie via de methode met behulp `LanguageUnderstandingModel FromEndpoint` van `verbose=true` de URI-parameter. Hiermee wordt het [GitHub-probleem #880.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues/880) Zie de bijgewerkte documentatie [hier.](./get-started-intent-recognition.md#add-a-languageunderstandingmodel-and-intents)
- **C++/C#/Java:** zorg ervoor dat uw spraakassistent of bot onmiddellijk stopt met luisteren. `DialogServiceConnector` ([C++](/cpp/cognitive-services/speech/dialog-dialogserviceconnector), [C#](/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector), [Java](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector)) heeft nu een `StopListeningAsync()` -methode bij `ListenOnceAsync()` . Hierdoor wordt audio-opname onmiddellijk gestopt en wordt er zonder fouten gewacht op een resultaat, waardoor het perfect is voor gebruik met 'nu stoppen' scenario's voor het indrukken van knoppen.
- **C++/C#/Java/JavaScript:** ervoor zorgen dat uw spraakassistent of bot beter reageert op onderliggende systeemfouten. `DialogServiceConnector` ([C++](/cpp/cognitive-services/speech/dialog-dialogserviceconnector), [C#](/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector), [Java](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector), [JavaScript](/javascript/api/microsoft-cognitiveservices-speech-sdk/dialogserviceconnector)) heeft nu een nieuwe `TurnStatusReceived` gebeurtenis-handler. Deze optionele gebeurtenissen komen overeen met elke oplossing op de bot en rapporteren uitvoeringsfouten wanneer ze optreden, bijvoorbeeld als gevolg van een onverhandelde uitzondering, time-out of netwerkuitval tussen Direct Line Speech en de [`ITurnContext`](/dotnet/api/microsoft.bot.builder.iturncontext) bot. `TurnStatusReceived` maakt het gemakkelijker om te reageren op foutsituaties. Als een bot bijvoorbeeld te lang duurt voor een back-updatabasequery (bijvoorbeeld om een product op te zoeken), kan de client het opnieuw proberen met de vraag 'Sorry, dat heb ik niet helemaal door, kunt u het opnieuw proberen' of iets `TurnStatusReceived` soortgelijks.
- **C++/C#:** Gebruik de Speech SDK op meer platforms. Het [Speech SDK NuGet-pakket](https://www.nuget.org/packages/Microsoft.CognitiveServices.Speech) biedt nu ondersteuning voor binaire windows ARM/ARM64-desktops (UWP werd al ondersteund) om de Speech-SDK nuttiger te maken voor meer computertypen.
- **Java:** heeft nu een methode die eerder per ongeluk is uitgesloten [`DialogServiceConnector`](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector) `setSpeechActivityTemplate()` van de taal. Dit is gelijk aan het instellen van de eigenschap en vraagt alle toekomstige Bot Framework-activiteiten die afkomstig zijn van de Direct Line Speech-service, de geleverde inhoud samen te voegen in hun `Conversation_Speech_Activity_Template` JSON-nettoladingen.
- **Java:** verbeterde debuggen op laag niveau. De [`Connection`](/java/api/com.microsoft.cognitiveservices.speech.connection) klasse heeft nu een `MessageReceived` gebeurtenis, vergelijkbaar met andere programmeertalen (C++, C#). Deze gebeurtenis biedt op laag niveau toegang tot binnenkomende gegevens van de service en kan nuttig zijn voor diagnostische gegevens en debuggen.
...
- **JavaScript:** verbeterde apparaatprestaties door de toegevoegde eigenschap van het tekenreeksbesturingselement voor websocket-compressie. Uit prestatieoverwegingen is websocket-compressie standaard uitgeschakeld. Dit kan opnieuw worden ingeslagen voor scenario's met lage bandbreedte. Meer informatie [kunt u hier vinden.](/javascript/api/microsoft-cognitiveservices-speech-sdk/propertyid) Hiermee wordt het [GitHub-probleem #242.](https://github.com/microsoft/cognitive-services-speech-sdk-js/issues/242)
- **JavaScript:** er is ondersteuning toegevoegd voor beoordeling van uitspraak om de spraakuiting te kunnen evalueren. Zie de snelstart [hier.](./how-to-pronunciation-assessment.md?pivots=programming-language-javascript)
...
- **C++**: Er is een probleem opgelost met garbageverzameling `DialogServiceConnector` met , dat het [GitHub-probleem #794](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues/794).
...
- **C++/C#/Java:** Er is een uitzondering opgelost waardoor een toepassing geen spraakautorisatie-token of activiteitssjabloon meer dan één keer kan instellen op een `DialogServiceConnector` .
- **C++/C#/Java:** Er is een recognizer-crash opgelost vanwege een racevoorwaarde in de noodtoestand.
- **JavaScript:** [`DialogServiceConnector`](/javascript/api/microsoft-cognitiveservices-speech-sdk/dialogserviceconnector) respecteerde niet eerder de optionele `botId` parameter die is opgegeven in `BotFrameworkConfig` 's factories. Hierdoor moest de queryreeksparameter handmatig worden ingesteld `botId` om een niet-standaardbot te gebruiken. De fout is gecorrigeerd en de waarden die aan de fabrieken worden geleverd, worden gehonoreerd en gebruikt, met inbegrip van `botId` `BotFrameworkConfig` de nieuwe en `fromHost()` `fromEndpoint()` toevoegingen. Dit geldt ook voor de `applicationId` parameter voor `CustomCommandsConfig` .
- **JavaScript:** [GitHub-probleem opgelost #881](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues/881), waardoor recognizer-object opnieuw kan worden gebruikt.
- **JavaScript:** er is een probleem opgelost waarbij de SKU meerdere keren in één TTS-sessie werd verzonden, waarbij `speech.config` bandbreedte werd verspild.
- **JavaScript:** vereenvoudigde foutafhandeling bij autorisatie van microfoons, zodat een beschrijvend bericht kan worden weergegeven wanneer de gebruiker geen microfooninvoer in de browser heeft toegestaan.
...
- **Python:** [GitHub-probleem opgelost met #870](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues/870), waardoor 'DeprecationWarning: the imp module is deprecated in favor of importlib' wordt verwijderd.
...
## <a name="speech-cli-also-known-as-spx-2021-january-release"></a>Speech CLI (ook wel SPX genoemd): release van 2021-januari
...
**COVID-19-verkorte** tests: Omdat de continue pandemie van onze technici blijft vereisen dat ze thuis werken, zijn handmatige verificatiescripts voor pre-pandemie aanzienlijk verminderd. We testen op minder apparaten met minder configuraties en de kans dat omgevingsspecifieke fouten worden opgelost, kan toenemen. We valideren nog steeds grondig met een grote set automatisering. Laat het ons weten op GitHub in het onwaarschijnlijke geval dat we iets [hebben gemist.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues?q=is%3Aissue+is%3Aopen)<br>
Blijf in orde!
...
- 5 nieuwe stemmen **in de preview-landverlening:** Kert in Het lets `et-EE` (Estisch), Colm in `ga-IE` Nuenen (Ierland), Nils in `lv-LV` Lets (Lets), Alsa's in `lt-LT` Deens (Rusland), Joseph in `mt-MT` Nuenen (Boens).
...
- Afstemming van de innatie ingeschakeld voor alle neurale stemmen in verschillende talen.
...
- Alle `zh-CN` neurale stemmen zijn bijgewerkt om Engels te ondersteunen.
- Schakel alle `zh-CN` neurale stemmen in om aanpassing van de intonatie te ondersteunen. SSML of audio-inhoud maken kan worden gebruikt om aan te passen voor de beste innation.
- Alle `zh-CN` neurale stemmen met meerdere stijlen zijn bijgewerkt om controle te `StyleDegree` ondersteunen. De intensiteit van emoties (soft of strong) is aanpasbaar.
...
- **Er worden vijf nieuwe stemmen en talen** geïntroduceerd in de portfolio neurale TTS. Dat zijn ze: Grace in Haddena (Italië), Ona in Het Letse (Rusland), Anu in HetEns (Rusland), Orla in Eues (Ierland) en Everita in Het Letse (Lets).
- **Er zijn `zh-CN` vijf nieuwe stemmen met meerdere stijlen** en rollen die ondersteuning bieden voor: Kunnenhan, Contactmo, Contactrui, Nuxxia en Yunxi.
...
- Met neurale TTS-containers kunnen ontwikkelaars spraaksynthese uitvoeren met de meest natuurlijke digitale stemmen in hun eigen omgeving voor specifieke vereisten voor beveiliging en gegevensbeheer. Controleer [hoe u Speech Containers installeert.](speech-container-howto.md) 

**Nieuwe functies**
- **Custom Voice:** stelt gebruikers in staat om een spraakmodel van de ene regio naar de andere te kopiëren; ondersteunde opzegging en hervatting van eindpunten. Ga hier naar [de portal.](https://speech.microsoft.com/customvoice)
- [Ondersteuning voor SSML-stiltetags.](speech-synthesis-markup.md#add-silence) 
- Algemene verbeteringen van de TTS-spraakkwaliteit: Verbeterde uitspraaknauwkeurigheid op woordniveau in nb-NO. Minder uitspraak van 53%.
...
**Nieuwe functies**
- Een nieuwe stijl wordt ondersteund in Den `newscast` Ouden. Bekijk [hoe u de spreekstijlen gebruikt in SSML](speech-synthesis-markup.md#adjust-speaking-styles).
- **Neurale stemmen zijn bijgewerkt naar HiFiNet vocoder, met een hogere audiokwaliteit en een snellere synthesesnelheid.** Dit is een voordeel voor klanten waarvan het scenario afhankelijk is van hi-fi audio of lange interacties, waaronder videosynchronisatie, audioboeken of onlinemateriaal voor onderwijs. [Lees meer over het verhaal en de stemvoorbeelden op onze blog van de tech-community](https://techcommunity.microsoft.com/t5/azure-ai/azure-neural-tts-upgraded-with-hifinet-achieving-higher-audio/ba-p/1847860)
- **[Custom Voice](https://speech.microsoft.com/customvoice)  &  [audio-inhoud maken Studio](https://speech.microsoft.com/audiocontentcreation) gelokaliseerd naar 17 locales.** Gebruikers kunnen de gebruikersinterface eenvoudig overschakelen naar een lokale taal voor een meer gebruiksvriendelijke ervaring.   
- **audio-inhoud maken:** Er is een besturingselement voor stijlvereniging toegevoegd voorXiaoNeural; De aangepaste onderbrekingsfunctie is verfijnd met incrementele onderbrekingen van 50 ms. 
...
- Verbeterde `ja-JP` leesing van één woord voor het woordenlijstscenario. Gereduceerde uitspraakfout met 80%.
- `zh-CN-XiaoxiaoNeural`: Verbeterde toonkwaliteit van sentiment/CustomerService/Newscast/Gaand/Stijl.
...
- **Python/Objective-C:** ondersteuning toegevoegd voor de `KeywordRecognizer` API. Documentatie vindt u [hier.](./custom-keyword-basics.md)
...
**Wijziging die een grote wijziging doorbreekt**
...
**Opgeloste fouten**
- **Alle:** regressie van 1,13 is opgelost waarbij `SetServiceProperty` waarden met bepaalde speciale tekens werden genegeerd.
- **C#**: Er zijn vaste Windows-consolevoorbeelden op Visual Studio 2019 mislukt om systeemeigen DLL's te vinden.
- **C#**: Er is een crash met geheugenbeheer opgelost als de stroom wordt gebruikt als `KeywordRecognizer` invoer.
- **ObjectiveC/Swift:** er is een crash met geheugenbeheer opgelost als stream wordt gebruikt als invoer van de kiezer.
- **Windows:** er is een probleem opgelost met het naast elkaar bestaan van BT HFP/A2DP in UWP.
...
- **JavaScript:** Er is een probleem opgelost met continue herkenning in Safari op macOS.
...
- **C++/C#/Java/Python/Swift/ObjectiveC:** Er is een potentieel opgelost bij het lezen `SPX_INVALID_ARG` van een bepaalde `AudioDataStream` positie.
...
- **C++/C#/Java/Python/Swift/ObjectiveC:** Hier voorbeeld toegevoegd voor beoordeling van [uitspraak](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/samples)
- **Xamarin:** quickstart bijgewerkt naar de meest recente Visual Studio sjabloon [hier](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/quickstart/csharp/xamarin).

**Bekend probleem**
- DigiCert Global Root G2-certificaat wordt standaard niet ondersteund in HoloLens 2 en Android 4.4 (KitKat) en moet worden toegevoegd aan het systeem om de Speech SDK functioneel te maken. Het certificaat wordt in de nabije HoloLens 2 aan de besturingssysteemafbeeldingen toegevoegd. Android 4.4-klanten moeten het bijgewerkte certificaat toevoegen aan het systeem.

**CoVID-19 verkorte tests:** Omdat we de afgelopen weken op afstand hebben gewerkt, konden we niet zoveel handmatige verificatietests uitvoeren als normaal. We hebben geen wijzigingen aangebracht die volgens ons alles zouden kunnen hebben verbroken en onze geautomatiseerde tests zijn allemaal geslaagd. Laat het ons weten op GitHub in het onwaarschijnlijke geval dat we iets [hebben gemist.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/issues?q=is%3Aissue+is%3Aopen)<br>
Blijf in orde!

## <a name="speech-cli-also-known-as-spx-2020-october-release"></a>Speech CLI (ook wel bekend als SPX): release van 2020-oktober
...
- `spx csr evaluation create|status|list|update|delete` : vergelijk nieuwe modellen met de basislijn-waarheid/andere modellen.
...
- `spx help` : verbeterde tekst en tekstkleur van de back-tick met code (blauw).
...
* **Custom Voice: er wordt een nieuwe functie uitgebracht om automatisch de kwaliteit van trainingsgegevens te controleren.** Wanneer u uw gegevens uploadt, onderzoekt het systeem verschillende aspecten van uw audio- en transcriptgegevens en worden problemen automatisch opgelost of gefilterd om de kwaliteit van het spraakmodel te verbeteren. Dit omvat het volume van uw audio, het niveau van ruis, de nauwkeurigheid van de uitspraak van spraak, de afstemming van spraak met de genormaliseerde tekst, stilte in de audio, naast de audio- en scriptindeling. 

* **audio-inhoud maken: een set nieuwe functies voor krachtigere** stemafstemming en audiobeheermogelijkheden.

    * Uitspraak: de functie voor het afstemmen van uitspraak is bijgewerkt naar de meest recente phoneme-set. U kunt het juiste phoneme-element kiezen uit de bibliotheek en de uitspraak verfijnen van de woorden die u hebt geselecteerd. 

    * Downloaden: De functie "Downloaden"/"Exporteren" is verbeterd ter ondersteuning van het genereren van audio per alinea. U kunt inhoud in hetzelfde bestand/SSML bewerken terwijl u meerdere audio-uitvoer genereert. De bestandsstructuur van 'Downloaden' is ook verfijnd. U kunt nu eenvoudig alle audiobestanden in één map krijgen. 

    * Taakstatus: de ervaring voor het exporteren van meerdere bestanden is verbeterd. Wanneer u meerdere bestanden in het verleden exporteert en een van de bestanden is mislukt, mislukt de hele taak. Maar nu worden alle andere bestanden met succes geëxporteerd. Het taakrapport wordt verrijkt met meer gedetailleerde en gestructureerde informatie. U kunt de logboeken nu controleren op alle mislukte bestanden en zinnen met het rapport. 

    * SSML-documentatie: gekoppeld aan het SSML-document om u te helpen de regels te controleren voor het gebruik van alle afstemmingsfuncties.

* **De Spraaklijst-API is bijgewerkt met een gebruiksvriendelijke** weergavenaam en de spreekstijlen die worden ondersteund voor neurale stemmen.

### <a name="general-tts-voice-quality-improvements"></a>Algemene verbeteringen in de spraakkwaliteit van TTS

* Gereduceerd uitspraakpercentage op woordniveau voor `ru-RU` (fouten gereduceerd met 56%) en `sv-SE` (fouten gereduceerd met 49%)

* Verbeterde polyogramie van het lezen van woorden `en-US` op neurale stemmen met 40%. Voorbeelden van veelhoekwoorden zijn onder andere 'read', 'live', 'content', 'record', 'object', enzovoort. 

* De natuurlijkheid van de vraagtoon in is `fr-FR` verbeterd. Winst mos (gemiddelde meningscore) : +0,28
...
* Een aantal fouten met het hulpprogramma audio-inhoud maken opgelost 
    * Probleem opgelost met automatisch vernieuwen. 
    * Problemen opgelost met spraakstijlen in zh-CN in de Azië - oost South.
...
Spraak-naar-tekst heeft 26 nieuwe landtalen uitgebracht in augustus: 2 Europese talen en , 5 Engelse landtalen en 19 Spaans land voor de meeste `cs-CZ` `hu-HU` Zuid-Amerikaanse landen. Hieronder vindt u een lijst met de nieuwe localen. Bekijk hier de volledige lijst [met talen.](./language-support.md)
...
| `es-BO` | Spaans (Argentinië)                 | 
| `es-CL` | Spaans (Argentinië)                   | 
| `es-CO` | Spaans (Columbia)                | 
| `es-CR` | Spaans (Hadoens)              | 
| `es-CU` | Spaans (Moet)                    | 
...
| `es-PA` | Spaans (Spanje)                  | 
| `es-PE` | Spaans (Cura)                    | 
| `es-PR` | Spaans (Verenigde Staten)             | 
| `es-PY` | Spaans (Argentinië)                | 
| `es-SV` | Spaans (El Hadoe)             | 
| `es-US` | Spaans (VS)                     | 
| `es-UY` | Spaans (Vietnam)                 | 
| `es-VE` | Spaans (Cura)               | 
| `hu-HU` | Hongaars (Hongarije)               | 


## <a name="speech-sdk-1130-2020-july-release"></a>Speech SDK 1.13.0: release van 2020-juli
...
* **Neurale TTS, 15 nieuwe neurale stemmen:** de nieuwe stemmen die zijn toegevoegd aan de portfolio neurale TTS zijn Salma in het Arabisch `ar-EG` (India), Zaremah in het Arabisch `ar-SA` (Arabisch) (Alba in `ca-ES` Token (Spanje), Moetel in Deens (Brazilië), Neerja in het Engels `da-DK` `es-IN` (India), Noora in `fi-FI` Fins (Fins) Hindi), Swara in `hi-IN` Hindi (India), Cotone in het Nederlands `nl-NL` (Nederland), Zofia in Pools (Rusland), Fernanda in het Portugees `pl-PL` `pt-PT` (Portugal), Dartone in Russisch `ru-RU` (Rusland), Hillevi in Zweeds `sv-SE` (Rusland), Moetara in `th-TH` Thai (Thai), HiuGaai in `zh-HK` het Chinees (Cantonese, Traditioneel) en HchinoYu in `zh-TW` het Chinees (Chinese Mandarijn). Controleer alle [ondersteunde talen.](./language-support.md#neural-voices)  

* Custom Voice gestroomlijnde spraaktests met de **trainingsstroom** om de gebruikerservaring te vereenvoudigen: met de nieuwe testfunctie wordt elke stem automatisch getest met een vooraf gedefinieerde testset die is geoptimaliseerd voor elke taal voor algemene scenario's en scenario's met spraakassistenten. Deze testsets worden zorgvuldig geselecteerd en getest om typische gebruiksgevallen en phonemes in de taal op te nemen. Bovendien kunnen gebruikers er nog steeds voor kiezen om hun eigen testscripts te uploaden bij het trainen van een model.
...
* **audio-inhoud maken: er wordt een set nieuwe functies uitgebracht om krachtigere mogelijkheden voor spraakafstemming en audiobeheer mogelijk te maken**
    ...
    * `Pitch`, en zijn verbeterd om afstemming te ondersteunen met een vooraf gedefinieerde waarde, zoals `rate` `volume` traag, gemiddeld en snel. Het is nu eenvoudig voor gebruikers om een 'constante' waarde te kiezen voor het bewerken van audio.
...
* Verbeterde TTS vocoder in voor een hogere betrouwbaarheid en lagere latentie.
    ...
    * Er is een update van Zalxiao in naar de nieuwe vocoder bijgewerkt met `zh-CN` +0148 CMOS-winst voor het algemene domein, +0,348 voor de newscast-stijl en +0,195 voor de stijl .195. 

* Bijgewerkte `de-DE` `ja-JP` modellen en spraakmodellen om de TTS-uitvoer natuurlijker te maken.
    
    * Katja in bijgewerkt met de meest recente `de-DE` prosody-modelleringsmethode, de MOS-winst (Mean Opinion Score) is +0,13. 
...
* Valuta lezen
    ...
    * Ondersteuning voor negatieve valuta (zoals "-325 €" ) in de volgende locales: `en-US` , , , , , , `en-GB` `fr-FR` `it-IT` `en-AU` `en-CA` .
...
- **JavaScript:** geoptimaliseerde implementatie van de browsermicrofoon die de nauwkeurigheid van spraakherkenning verbetert.
- **Java:** herfactored bindingen met behulp van directe JNI-implementatie zonder SNIA. Deze wijziging vermindert de bindingsgrootte met 10x voor alle Java-pakketten die worden gebruikt voor Windows, Android, Linux en Mac en vereenigt de verdere ontwikkeling van de Speech SDK Java-implementatie.
...
- Er is een probleem opgelost waarbij de reden van het resultaat niet juist was `IntentRecognizer` ingesteld bij het herkennen van intenties van eenvoudige zinnen.
...
- Geheugenlekken in de engine voor sleutelwoord herkennen zijn opgelost.
...
**Opgeloste fouten**
 ...
 - Er is een fout opgelost waarbij gesprekstranscriber niet goed moest wachten in JAVA-API's 
...
- JavaScript: Er is ondersteuning toegevoegd `FromHost API` voor om het gebruik van on-prem-containers en onafhankelijke clouds te gemaken. Zie de documentatie [hier.](speech-container-howto.md)
...
- Quickstart-voorbeelden zijn [hier opgelost.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/quickstart/cpp) Deze waren afsluitend zonder de uitzondering 'microfoon niet gevonden' weer te geven in Linux, macOS, Windows.
...
## <a name="speech-sdk-180-2019-november-release"></a>Speech SDK 1.8.0: release van 2019 tot november
...
- `DialogServiceConnector` sessiestarts bieden nu op de juiste wijze gebeurtenissen, waaronder bij het aanroepen `ListenOnceAsync()` tijdens een actieve `StartKeywordRecognitionAsync()`
...
- Voorkom dat het webpakket https-proxy-agent laadt.
...
- JavaScript: foutieve statusrapportage voor spraak is beëindigd op RequestSession opgelost.
...
- De Speech SDK ondersteunt nu Unity in een bètaversie. Geef feedback via de sectie probleem in de [GitHub-voorbeeldopslagplaats](https://aka.ms/csspeech/samples). Deze release ondersteunt Unity in Windows x86 en x64 (desktop- of Universeel Windows-platform-toepassingen) en Android (ARM32/64, x86). Meer informatie vindt u in onze [Unity-snelstart.](./get-started-speech-to-text.md?pivots=programming-language-csharp&tabs=unity)
...
- Nieuw voorbeeld voor het gebruik van [de batchtranscriptie REST API](batch-transcription.md).
...
- Javascript
  - Voeg ondersteuning toe voor het `Connection` -object.
  - Typedefinitiebestanden voor gebundelde JavaScript toevoegen
...
- Als u een recognizer `FromEndpoint` maakt, kunt u parameters rechtstreeks toevoegen aan de eindpunt-URL. Met `FromEndpoint` behulp van kunt u de recognizer niet configureren via de standaardconfiguratie-eigenschappen.
...
- SessionId's die door de SDK zijn gemaakt, zijn niet altijd willekeurig voor bepaalde &nbsp; talen/omgevingen. Initialisatie van willekeurige generator toegevoegd om dit probleem op te lossen.
- Verwerking van autorisatie-token verbeteren. Als u een autorisatie-token wilt gebruiken, geeft u op in de `SpeechConfig` en laat u de abonnementssleutel leeg. Maak vervolgens de kiezer zoals gebruikelijk.
...
- Er is een fout in de audiopomp opgelost die de volgende verzendplanning niet heeft gepland als de huidige verzend mislukt.
- Continue herkenning herstellen met een auth-token.
- Opgeloste fout voor verschillende kiezer/eindpunten.
- Verbeteringen in de documentatie.

## <a name="speech-sdk-120-2018-december-release"></a>Speech SDK 1.2.0: release van 2018 tot december

**Nieuwe functies**

- Python
  - De bètaversie van Python-ondersteuning (3.5 en hoger) is beschikbaar in deze release. Zie hier voor meer informatie](quickstart-python.md).
- Javascript
  ...
  - De lengtebeperking voor audiosessies is verwijderd. Opnieuw verbinden vindt automatisch plaats onder de omslag.
- `Connection` Object
...
- Verbeteringen in het gebruik van interne threads, waardoor het aantal threads, vergrendelingen en dempen wordt beperkt.
- Verbeterde foutrapportage /informatie. In verschillende gevallen zijn foutberichten niet helemaal naar buiten doorgegeven.
- Ontwikkelingsafhankelijkheden in JavaScript bijgewerkt om gebruik te maken van bijgewerkte modules.

**Opgeloste fouten**

- Geheugenlekken opgelost omdat het type niet overeen komt in `RecognizeAsync` .
- In sommige gevallen werden uitzonderingen gelekt.
- Geheugenlekken in vertaalgebeurtenisargumenten herstellen.
...
- Verbeterde foutcode en -berichten. Als een herkenning een fout retourneert, is deze al ingesteld (in de geannuleerde gebeurtenis) of `Reason` `CancellationDetails` (in herkenningsresultaat) op `Error` . De geannuleerde gebeurtenis bevat nu twee extra leden, `ErrorCode` en `ErrorDetails` . Als de server aanvullende foutinformatie heeft geretourneerd met de gerapporteerde fout, is deze nu beschikbaar in de nieuwe leden.
...
- Er zijn verschillende uitzonderingen opgelost die zijn gevonden in recognizers. Bovendien worden uitzonderingen af en geconverteerd naar `Canceled` een gebeurtenis.
...
- Er is een fout opgelost waardoor het `EndpointId` (in sommige gevallen) niet mogelijk was om de in JavaScript in te stellen.
- Parameterorder gewijzigd in AddIntent in JavaScript en ontbrekende `AddIntent` JavaScript-handtekening toegevoegd.

**Voorbeelden**

- C++ en C#-voorbeelden toegevoegd voor pull- en pushstreamgebruik in de [voorbeeldopslagplaats](https://aka.ms/csspeech/samples).

## <a name="speech-sdk-101"></a>Speech SDK 1.0.1

Verbeteringen in de betrouwbaarheid en oplossingen voor fouten:

- Mogelijke fatale fout opgelost vanwege racevoorwaarde bij het uitstellen van de recognizer
- Mogelijke onveilige fout opgelost bij niet-beveiligde eigenschappen.
- Aanvullende fout- en parametercontrole toegevoegd.
- Objective-C: Mogelijke fatale fout opgelost die wordt veroorzaakt door het overschrijven van de naam in NSString.
...
- Ondersteuning voor Objective-C in iOS. Bekijk onze [Objective-C-quickstart voor iOS.](https://github.com/Azure-Samples/cognitive-services-speech-sdk/tree/master/quickstart/objectivec/ios/from-microphone)
...
- Er is een fout opgelost die inactieve threads en een groter aantal open en ongebruikte sockets zou kunnen veroorzaken.
- Er is een probleem opgelost waarbij een langdurige herkenning tijdens de overdracht kon worden beëindigd.
...
- Ondersteuning voor Android-platform (API 23: Android 6.0 Marshmallow of hoger). Bekijk de [Android-snelstart](./get-started-speech-to-text.md?pivots=programming-language-java&tabs=android).
...
- Gedetailleerde uitvoerindeling
  ...
  Wanneer u een `SpeechRecognizer` maakt, kunt u een `Detailed` aanvraag- of `Simple` uitvoerindeling aanvragen. De bevat een betrouwbaarheidsscore, herkende tekst, onbewerkte lexicale vorm, genormaliseerde vorm en genormaliseerde vorm met `DetailedSpeechRecognitionResult` gemaskeerd grof taalgebruik.
...
- Als een recognizer een audio-invoerbestand heeft verbruikt, houdt deze de bestandsingang langer vast dan nodig is.
