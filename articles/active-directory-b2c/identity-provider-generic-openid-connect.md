...
Met het bereik worden de informatie en machtigingen gedefinieerd die u wilt verzamelen van uw ID-provider, bijvoorbeeld `openid profile` . Als u het ID-token van de ID-provider wilt ontvangen, moet u het `openid` bereik opgeven. Zonder het ID-token kunnen gebruikers zich niet aanmelden bij Azure AD B2C met behulp van de aangepaste ID-provider. Andere bereiken kunnen worden toegevoegd, gescheiden door een spatie. Raadpleeg de documentatie van de aangepaste ID-provider om te zien welke andere bereiken mogelijk beschikbaar zijn.
