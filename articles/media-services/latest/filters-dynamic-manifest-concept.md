...
Azure Media Services ondersteunt HLS-, MPEG-DASH-en Smooth Streaming-protocollen. Als onderdeel van [dynamische pakketten](encode-dynamic-packaging-concept.md)worden de streaming-client manifesten (HLS Master-afspeel lijst, Dash-media Presentation Description [MPD] en Smooth streaming) dynamisch gegenereerd op basis van de indelings kiezer in de URL. Zie voor meer informatie de bezorgings protocollen in de [algemene werk stroom op aanvraag](encode-dynamic-packaging-concept.md#to-prepare-your-source-files-for-delivery).
...
In het volgende voor beeld werd een encoder gebruikt voor het coderen van een mezzanine-asset in zeven ISO Mp4's-video weergaven (van 180p naar 1080p). De versleutelde Asset kan [dynamisch worden verpakt](encode-dynamic-packaging-concept.md) in een van de volgende streaming-protocollen: HLS, MPEG Dash en Smooth.
