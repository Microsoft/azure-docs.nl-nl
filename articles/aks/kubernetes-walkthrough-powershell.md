...
* Implementeer een AKS-cluster met behulp van Power shell. 
...
Als u Power shell lokaal uitvoert, installeert u de AZ Power shell-module en maakt u verbinding met uw Azure-account met behulp van de cmdlet [Connect-AzAccount](/powershell/module/az.accounts/Connect-AzAccount) . Zie [Azure PowerShell installeren][install-azure-powershell] voor meer informatie over het installeren van de Az-module van PowerShell.

[!INCLUDE [cloud-shell-try-it](../../includes/cloud-shell-try-it.md)]

Als u meerdere Azure-abonnementen hebt, selecteert u de juiste abonnements-ID waarin de resources moeten worden gefactureerd met de cmdlet [set-AzContext](/powershell/module/az.accounts/set-azcontext) .
...
Maak een resourcegroep met behulp van de cdmlet [New-AzResourceGroup][new-azresourcegroup].
...
1. Genereer een SSH-sleutel paar met behulp van het `ssh-keygen` opdracht regel programma. 
    * Zie voor meer informatie [Snelle stappen: een sleutelpaar met een openbare-persoonlijke SSH-sleutel voor virtuele Linux-machines maken en gebruiken](../virtual-machines/linux/mac-create-ssh-keys.md).

1. Maak een AKS-cluster met behulp van de cmdlet [New-AzAks][new-azaks] . Azure Monitor voor containers is standaard ingeschakeld.
...
1. `kubectl`Lokaal installeren met behulp van de `Install-AzAksKubectl` cmdlet:

    ```azurepowershell
    Install-AzAksKubectl
    ```

2. Configureer `kubectl` om verbinding te maken met uw Kubernetes-cluster met behulp van de cmdlet [import-AzAksCredential][import-azakscredential] . Met de volgende cmdlet worden referenties gedownload en wordt de Kubernetes CLI geconfigureerd om deze te gebruiken.
...
Bekijk de metrische gegevens voor de status van de cluster knooppunten die zijn vastgelegd door Azure Monitor voor containers in de Azure Portal. 

## <a name="delete-the-cluster"></a>Het cluster verwijderen

Opschonen van uw overbodige resources om Azure-kosten te voor komen. Gebruik de cmdlet [Remove-AzResourceGroup][remove-azresourcegroup] om de resource groep, de container service en alle gerelateerde resources te verwijderen.
