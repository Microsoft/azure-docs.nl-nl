...
Hiermee worden de waarden opgehaald van alle uitvoer kolommen voor een stroom. U kunt een optionele naam van een stream door geven als het tweede argument.   
* ``columns()``
...
___
### <code>mapIf</code>
<code><b>mapIf (<value1> : array, <value2> : binaryfunction, <value3>: binaryFunction) => any</b></met code><BR/><BR/> voorwaardelijk wijst een matrix toe aan een andere matrix met dezelfde of een kleinere lengte. De waarden kunnen van elk gegevens type zijn, waaronder structTypes. Er wordt een toewijzings functie gebruikt waarbij u het item in de matrix als #item en huidige index als #index kunt adresseren. Voor diep geneste kaarten kunt u verwijzen naar het haakje t maps using the ``#item_[n](#item_1 , #index_1...)`` notation.
...
___
### <code>mapLoop</code>
<code><b>mapLoop(<value1> : integer, <value2> : unaryfunction) => any</b></code><BR/><BR/> lussen door lopen van 1 tot lengte om een matrix van die lengte te maken. Er wordt een toewijzings functie gebruikt waarbij u de index in de matrix als #index kunt adresseren. Voor diep geneste kaarten kunt u verwijzen naar de bovenliggende kaarten met behulp van de #index_n (#index_1, #index_2...) notation.
...
Retourneert een hexadecimale teken reeks representatie van een binaire waarde * ``hex(toBinary([toByte(0x1f), toByte(0xad), toByte(0xbe)])) -> '1fadbe'``
___
### <code>unhex</code>
<code><b>unhex(<value1>: string) => binary</b></code><br/><br/>
Unhexes een binaire waarde uit de teken reeks representatie. Dit kan worden gebruikt in combi natie met SHA2, MD5 voor conversie van teken reeks naar binaire weer gave *   ``unhex('1fadbe') -> toBinary([toByte(0x1f), toByte(0xad), toByte(0xbe)])``
