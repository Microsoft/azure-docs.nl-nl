...
> [!NOTE]
> Het is raadzaam om het [algemene waarschuwings schema](../alerts/alerts-common-schema.md) voor uw webhook-integraties te gebruiken. Het algemene waarschuwings schema biedt het voor deel dat er één uitbreid bare en Unified alert Payload is voor alle waarschuwings Services in Azure Monitor. Voor regels voor logboek waarschuwingen waarvoor een aangepaste JSON-nettolading is gedefinieerd, wordt het payload-schema teruggezet naar het schema dat [hier](../alerts/alerts-common-schema-definitions.md#log-alerts)wordt beschreven. Dit betekent dat als u een aangepaste JSON-nettolading wilt definiëren, de webhook het algemene waarschuwings schema niet kan gebruiken. Waarschuwingen waarvoor het algemene schema is ingeschakeld, hebben een maximale grootte van 256 KB per waarschuwing. een grotere waarschuwing bevat geen Zoek resultaten. Wanneer de zoek resultaten niet zijn opgenomen, moet u de `LinkToFilteredSearchResultsAPI` of gebruiken `LinkToSearchResultsAPI` om de query resultaten te openen via de log Analytics-API.
