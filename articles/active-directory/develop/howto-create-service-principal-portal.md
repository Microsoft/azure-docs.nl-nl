...
description: Maak een nieuwe Azure Active Directory app & service-principal voor het beheren van de toegang tot resources met op rollen gebaseerd toegangsbeheer in Azure Resource Manager.
...
In dit artikel wordt beschreven hoe u een nieuwe Azure Active Directory (Azure AD)-toepassing en service-principal maakt die kunnen worden gebruikt met op rollen gebaseerd toegangsbeheer. Wanneer u toepassingen, gehoste services of geautomatiseerde hulpprogramma's hebt die toegang moeten hebben tot of resources moeten wijzigen, kunt u een identiteit voor de app maken. Deze identiteit staat bekend als een service-principal. Toegang tot resources wordt beperkt door de rollen die zijn toegewezen aan de service-principal, zodat u kunt bepalen welke resources toegankelijk zijn en op welk niveau. Uit veiligheidsoverwegingen is het altijd aanbevolen om voor geautomatiseerde tools service-principals te gebruiken, in plaats van deze zich te laten aanmelden met een gebruikers-id.

In dit artikel wordt beschreven hoe u de portal gebruikt om de service-principal te maken in de Azure Portal. Het richt zich op een toepassing met één tenant waarin de toepassing is bedoeld om binnen slechts één organisatie te worden uitgevoerd. Normaal gesproken gebruikt u toepassingen met één tenant voor Line-Of-Business-toepassingen die binnen uw organisatie worden uitgevoerd.  U kunt ook [een Azure PowerShell om een service-principal te maken.](howto-authenticate-service-principal-powershell.md)
...
Laten we meteen aan de zelfmakende identiteit gaan doen. Als er een probleem is, controleert u de [vereiste machtigingen](#permissions-required-for-registering-an-app) om ervoor te zorgen dat uw account de identiteit kan maken.
...
Als u toegang wilt krijgen tot resources in uw abonnement, moet u een rol toewijzen aan de toepassing. Bepaal welke rol de juiste machtigingen voor de toepassing biedt. Zie Ingebouwde [Azure-rollen](../../role-based-access-control/built-in-roles.md)voor meer informatie over de beschikbare rollen.
...
1. Selecteer in Azure Portal bereikniveau aan wie u de toepassing wilt toewijzen. Als u bijvoorbeeld een rol wilt toewijzen op abonnementsbereik, zoekt en **selecteert** u Abonnementen of **selecteert** u Abonnementen op de **startpagina.**
...
Uw service-principal is ingesteld. U kunt deze gaan gebruiken om uw scripts of apps uit te voeren. Ga naar Bedrijfstoepassingen om uw service-principal te beheren (machtigingen, machtigingen, toestemming van de gebruiker, te zien welke gebruikers toestemming hebben gegeven, machtigingen te controleren, aanmeldingsgegevens te bekijken, en meer), naar **Bedrijfstoepassingen te gaan.**

In de volgende sectie ziet u hoe u waarden op kunt halen die nodig zijn bij het programmatisch aanmelden.

## <a name="get-tenant-and-app-id-values-for-signing-in"></a>Waarden voor tenant- en app-id's voor aanmelden op halen
...
1. Kopieer de map-id (tenant-id) en sla deze op in uw toepassingscode.
    ...
    De map-id (tenant-id) vindt u ook op de standaardoverzichtspagina van de directory.
...
Exporteert dit certificaat naar [](/dotnet/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in) een bestand met de MMC-module Gebruikerscertificaat beheren die toegankelijk is vanuit de Windows-Configuratiescherm.
...
1. Klik met de rechtermuisknop op het certificaat dat u hebt gemaakt en selecteer **Alle taken->Exporteren.**
...
Houd er rekening mee dat u mogelijk aanvullende machtigingen moet configureren voor resources die uw toepassing nodig heeft om toegang te krijgen. U moet bijvoorbeeld ook het [toegangsbeleid](../../key-vault/general/security-overview.md#privileged-access) van een sleutelkluis bijwerken om uw toepassing toegang te geven tot sleutels, geheimen of certificaten.
...
1. Selecteer **Toevoegen om** het toegangsbeleid toe te voegen en selecteer vervolgens Opslaan **om** uw wijzigingen door te voeren.
...
* Zie bewerkingen voor resourceproviders voor een lijst met beschikbare acties die aan gebruikers [kunnen Azure Resource Manager worden verleend of geweigerd.](../../role-based-access-control/resource-provider-operations.md)
* Zie de naslaginformatie over de Api voor toepassingen [](/graph/api/resources/application) voor meer informatie over het werken **met** app-Microsoft Graph.
