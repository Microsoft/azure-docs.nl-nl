...
|Metrisch|Exporteerbaar via Diagnostische instellingen?|Weergave naam voor metrische gegevens|Eenheid|Aggregatietype|Beschrijving|Dimensies|
|---|---|---|---|---|---|---|
|RequestLatency|Ja|Latentie van aanvraag|Milliseconden|Totaal|De tijd die de server nodig heeft om de aanvraag te verwerken|Bewerking, authenticatie, protocol|
|RequestsTraffic|Ja|Vraagt verkeer|Percentage|Count|Aantal aanvragen dat is gedaan|Bewerking, authenticatie, protocol, status code, StatusCodeClass|
...
|usedmemoryRss|Ja|Gebruikte geheugen-RSS|Bytes|Maximum||InstanceId|
...
|OriginHealthPercentage|Ja|Oorsprong status percentage|Percentage|Gemiddeld|Het percentage geslaagde status tests van AFDX naar back-end.|Oorsprong, OriginPool|
...
|ServiceApiErrors|Ja|Service fouten|Count|Sum|Het totale aantal interne server fouten dat is gegenereerd door de service.|Protocol, verificatie, bewerking, resource type, status code, StatusCodeClass, StatusCodeText|
|ServiceApiLatency|Ja|Service latentie|Milliseconden|Gemiddeld|De reactie latentie van de service.|Protocol, verificatie, bewerking, resource type, status code, StatusCodeClass, StatusCodeText|
|ServiceApiRequests|Ja|Service Requests|Count|Sum|Het totale aantal aanvragen dat door de service is ontvangen.|Protocol, verificatie, bewerking, resource type, status code, StatusCodeClass, StatusCodeText|
...
|Metrisch|Exporteerbaar via Diagnostische instellingen?|Weergave naam voor metrische gegevens|Eenheid|Aggregatietype|Beschrijving|Dimensies|
|---|---|---|---|---|---|---|
|ApiLatency|Nee|ApiLatency|6|0|Meet latentie van API-aanvragen voor micro soft. IoTSpaces in milliseconden|Geen dimensies|
|FunctionExecutionLatency|Nee|FunctionExecutionLatency|6|0|Meet latentie van door de gebruiker gedefinieerde functie-uitvoering in milliseconden voor micro soft. IoTSpaces|Geen dimensies|
|MessageEgressFailure|Nee|MessageEgressFailure|2|3|Zoekt naar een gelokaliseerde teken reeks die vergelijkbaar is met het aantal mislukte metingen gebeurtenissen in Count voor micro soft. IoTSpaces|Geen dimensies|
|MessageEgressLatency|Nee|MessageEgressLatency|6|0|Hiermee wordt de latentie van de verzender naar andere eind punten in milliseconden voor micro soft. IoTSpaces gemeten.|Geen dimensies|
|MessageEgressSuccess|Nee|MessageEgressSuccess|2|3|Zoekt naar een gelokaliseerde teken reeks die vergelijkbaar is met het aantal voltooide tellingen gebeurtenissen in micro soft. IoTSpaces|Geen dimensies|
|ProcessingLatency|Nee|ProcessingLatency|6|0|Meet latentie van berichten die zijn opgenomen in een verzonden gebeurtenis in milliseconden voor micro soft. IoTSpaces|Geen dimensies|
...
|Metrisch|Exporteerbaar via Diagnostische instellingen?|Weergave naam voor metrische gegevens|Eenheid|Aggregatietype|Beschrijving|Dimensies|
|---|---|---|---|---|---|---|
|CPU|Ja|CPU-gebruik|Percentage|Gemiddeld|CPU-gebruik voor Premium streaming-eind punten. Deze gegevens zijn niet beschikbaar voor standaard streaming-eind punten.|VmId|
|Uitgaand verkeer|Ja|Uitgaand verkeer|Bytes|Totaal|De hoeveelheid uitgangs gegevens, in bytes.|OutputFormat|
|EgressBandwidth|Nee|Uitgangs band breedte|BitsPerSecond|Gemiddeld|Uitgangs band breedte in bits per seconde.|VmId|
...
|CbsVolumeBackupActive|Ja|Is volume back-up onderbroken|Count|Gemiddeld|Is het back-upbeleid opgeschort voor het volume? 1 Indien ja, 0 als Nee.|Geen dimensies|
...
|ExpressRouteGatewayCpuUtilization|Ja|CPU-gebruik (preview-versie)|Count|Gemiddeld|CPU-gebruik van de ExpressRoute-gateway|roleInstance|
...
|ExpressRouteGatewayPacketsPerSecond|Nee|Pakketten per seconde (preview-versie)|CountPerSecond|Gemiddeld|Aantal pakketten van de ExpressRoute-gateway|roleInstance|
...
|Metrisch|Exporteerbaar via Diagnostische instellingen?|Weergave naam voor metrische gegevens|Eenheid|Aggregatietype|Beschrijving|Dimensies|
|---|---|---|---|---|---|---|
|ScanCancelled|Ja|De scan is geannuleerd|Aantal|Totaal|Hiermee wordt het aantal geannuleerde scans aangegeven.|ResourceId|
|ScanCompleted|Ja|De scan is voltooid|Aantal|Totaal|Hiermee wordt het aantal scans aangegeven dat is voltooid.|ResourceId|
|ScanFailed|Ja|Kan niet scannen|Aantal|Totaal|Hiermee wordt aangegeven dat het aantal mislukte scans is mislukt.|ResourceId|
|ScanTimeTaken|Ja|Gebruikte scan tijd|Seconden|Totaal|Geeft de totale Scan tijd in seconden.|ResourceId|
...
|CpuTime|Ja|CPU-tijd|Seconden|Totaal|De hoeveelheid CPU die wordt verbruikt door de app, in seconden. Voor meer informatie over deze metrische gegevens. Zie https://docs.microsoft.com/azure/app-service/web-sites-monitor#cpu-time-vs-cpu-percentage (CPU-tijd versus CPU-percentage). Niet van toepassing op Azure Functions.|Exemplaar|
