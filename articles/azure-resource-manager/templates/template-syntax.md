...
| type |Yes |Type parameter waarde. De toegestane typen en waarden zijn **String**, **securestring**, **int**, **BOOL**, **object**, **secureObject** en **array**. Zie [gegevens typen in arm-sjablonen](data-types.md). |
...
| beschrijving |No |Beschrijving van de para meter die wordt weer gegeven voor gebruikers via de portal. Zie [opmerkingen in sjablonen](#comments)voor meer informatie. |
...
| functie naam |Yes |De naam van de aangepaste functie. Wanneer u de functie aanroept, moet u de naam van de functie combi neren met de naam ruimte. Als u bijvoorbeeld een functie wilt aanroepen met `uniqueName` de naam in de naam ruimte contoso, gebruikt u `"[contoso.uniqueName()]"` . |
...
| scope | No | De eigenschap scope is alleen beschikbaar voor [uitbreidings bron typen](../management/extension-resource-types.md). Gebruik deze methode wanneer u een bereik opgeeft dat verschilt van het implementatie bereik. Zie [bereik instellen voor uitbreidings resources in arm-sjablonen](scope-extension-resources.md). |
...
| kopiëren |No | Wordt gebruikt om meer dan één waarde voor een uitvoer te retour neren. **Waarde** opgeven of **kopiëren**. Zie [uitvoer iteratie in arm-sjablonen](copy-outputs.md)voor meer informatie. |
