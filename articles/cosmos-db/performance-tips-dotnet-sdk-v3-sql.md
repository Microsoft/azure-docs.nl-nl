...
- **Voor op VSTest gebaseerde test projecten**: Selecteer in het menu **test** van Visual Studio test  >  **instellingen** testen en stel de **standaard architectuur** van de processor in op **x64**.
...
De standaard verbindings modus van .NET v3 SDK is direct met het TCP-protocol. U configureert de verbindings modus wanneer u het `CosmosClient` exemplaar maakt in `CosmosClientOptions` .  Zie het artikel [connectiviteits modi](sql-sdk-connection-modes.md) voor meer informatie over de verschillende connectiviteits opties.
...
Zie [RetryAfter](/dotnet/api/microsoft.azure.cosmos.cosmosexception.retryafter#Microsoft_Azure_Cosmos_CosmosException_RetryAfter)voor meer informatie.
