...
Met ingang van SAP HANA 2,0 SPS 01 SAP is de installatie van actief/Lees functionaliteit toegestaan voor SAP HANA systeem replicatie, waarbij de secundaire systemen van SAP HANA systeem replicatie actief kunnen worden gebruikt voor lees-intensieve workloads. Ter ondersteuning van dergelijke installatie in een cluster is een tweede virtueel IP-adres vereist, waarmee clients toegang kunnen krijgen tot de secundaire SAP HANA data base met lees functionaliteit. Om ervoor te zorgen dat de secundaire replicatie site nog steeds kan worden geopend nadat er een overname is uitgevoerd, moet het cluster het virtuele IP-adres verplaatsen met de secundaire SAPHana-resource.

In deze sectie worden de extra stappen beschreven die nodig zijn voor het beheren van de systeem replicatie HANA actief/gelezen in een SUSE-cluster met hoge Beschik baarheid met een tweede virtueel IP-adres.    
Voordat u verder gaat, moet u ervoor zorgen dat er volledig geconfigureerde SUSE-beschik baarheids cluster SAP HANA-data base wordt beheerd, zoals beschreven in de bovenstaande segmenten van de documentatie.  
...
1. Voor **standaard** Load Balancer volgt u de extra stappen hieronder op dezelfde Load Balancer die u eerder hebt gemaakt in de vorige sectie.
   ...
   - Selecteer **OK**.
   - Nadat de nieuwe front-end-IP-groep is gemaakt, noteert u het frontend-IP-adres.
