...
ms.locfileid: "107210513"
---
# <a name="replicate-data-into-azure-database-for-mysql"></a>Gegevens repliceren naar Azure Database for MySQL

Met Replicatie van inkomende gegevens kunt u gegevens van een externe MySQL-server synchroniseren met de Azure Database for MySQL-service. De externe server kan on-premises, in virtuele machines of een database service worden gehost door andere cloud providers. Replicatie van inkomende gegevens is gebaseerd op de locatie van het binaire logboek bestand (binlog) of op gtid gebaseerde replicatie, native naar MySQL. Meer informatie over binlog-replicatie vindt u in het [overzicht van MySQL binlog-replicatie](https://dev.mysql.com/doc/refman/5.7/en/binlog-replication-configuration-overview.html). 
...
- Als SSL is ingeschakeld op de bron server, moet u ervoor zorgen dat het SSL-CA-certificaat dat is opgegeven voor het domein, is opgenomen in de `mysql.az_replication_change_master` of `mysql.az_replication_change_master_with_gtid` opgeslagen procedure. Raadpleeg de volgende [voor beelden](./howto-data-in-replication.md#4-link-source-and-replica-servers-to-start-data-in-replication) en de `master_ssl_ca` para meter.
