...
ms.locfileid: "107812656"
---
# <a name="security-considerations-for-azure-container-instances"></a>Beveiligingsoverwegingen voor Azure Container Instances

In dit artikel worden beveiligingsoverwegingen beschreven voor het gebruik van Azure Container Instances om container-apps uit te voeren. Dit zijn een aantal van de onderwerpen:
...
Een openbaar beschikbare containerafbeelding biedt geen garantie voor beveiliging. Container-afbeeldingen bestaan uit meerdere softwarelagen en elke softwarelaag kan beveiligingsproblemen hebben. Als u de bedreiging van aanvallen wilt verminderen, moet u afbeeldingen opslaan en ophalen uit een persoonlijk register, zoals Azure Container Registry of Docker Trusted Registry. Naast het leveren van een beheerd privéregister, Azure Container Registry ondersteuning voor verificatie op basis van [service-principals](../container-registry/container-registry-authentication.md) via Azure Active Directory voor basisverificatiestromen. Deze verificatie omvat op rollen gebaseerde toegang voor alleen-lezen (pull), schrijven (push) en andere machtigingen.
...
Nieuwe beveiligingsproblemen worden voortdurend ontdekt, dus het scannen op en identificeren van beveiligingsproblemen is een doorlopend proces. Scannen op beveiligingsleed opnemen in de levenscyclus van de container:
...
* Ga door met het scannen van containerafbeeldingen in het register om eventuele fouten te identificeren die tijdens de ontwikkeling zijn gemist en om eventuele nieuwe beveiligingsproblemen op te pakken die mogelijk aanwezig zijn in de code die wordt gebruikt in de containerafbeeldingen.  
...
Er is voldoende verandering en veranderlijkheid in een containerecosysteem zonder ook onbekende containers toe te staan. Alleen goedgekeurde containerafbeeldingen toestaan. Gebruik hulpprogramma's en processen om het gebruik van niet-goedgekeurde containerafbeeldingen te controleren en te voorkomen. 

Een effectieve manier om het aanvalsoppervlak te verminderen en te voorkomen dat ontwikkelaars kritieke beveiligingsfouten maken, is door de stroom van containerafbeeldingen in uw ontwikkelomgeving te controleren. U kunt bijvoorbeeld één Linux-distributie als basisafbeelding goed te staan, bij voorkeur een distributie die lean is (Alpine of CoreOS in plaats van Ubuntu), om de kans op potentiële aanvallen te minimaliseren. 

Ondertekening van afbeeldingen of vingerafdrukken kan een controleketen bieden waarmee u de integriteit van de containers kunt controleren. Zo ondersteunt Azure Container Registry docker-model voor [](https://docs.docker.com/engine/security/trust/content_trust) inhoud vertrouwen, waarmee uitgevers van afbeeldingen afbeeldingen kunnen ondertekenen die naar een register worden pushen en gebruikers van afbeeldingen alleen ondertekende afbeeldingen kunnen pullen.
...
Een onderdeel van het beheren van de beveiliging gedurende de levenscyclus van de container is het garanderen van de integriteit van de containerafbeeldingen in het register en wanneer ze worden gewijzigd of geïmplementeerd in productie. 

* Afbeeldingen met beveiligingsproblemen, zelfs kleine, mogen niet worden uitgevoerd in een productieomgeving. In het ideale ideale moment moeten alle in productie geïmplementeerde afbeeldingen worden opgeslagen in een privéregister dat toegankelijk is voor slechts enkele. Houd het aantal productie-afbeeldingen klein om ervoor te zorgen dat ze effectief kunnen worden beheerd.

* Omdat het moeilijk is om de oorsprong van software uit een openbaar beschikbare containerafbeelding vast te stellen, kunt u afbeeldingen maken op basis van de bron om kennis van de oorsprong van de laag te garanderen. Wanneer er een beveiligingsprobleem aan het licht komt in een zelf gebouwde containerinstallatiekopie, kunnen klanten sneller een oplossing vinden. Met een openbare afbeelding moeten klanten de hoofdmap van een openbare afbeelding vinden om deze te herstellen of een andere beveiligde afbeelding van de uitgever te krijgen. 
...
* Een grondig gescande afbeelding die in productie is geïmplementeerd, is niet gegarandeerd up-to-date voor de levensduur van de toepassing. Er kunnen beveiligingsproblemen worden gerapporteerd voor lagen van de installatiekopie die niet bekend waren of die na de implementatie naar productie zijn geïntroduceerd. 
  ...
  Controleer regelmatig afbeeldingen die in productie zijn geïmplementeerd om afbeeldingen te identificeren die verouderd zijn of al een tijdje niet zijn bijgewerkt. U kunt blauw-groene implementatiemethoden en -rolling upgrade gebruiken om containerafbeeldingen zonder downtime bij te werken. U kunt afbeeldingen scannen met behulp van hulpprogramma's die in de vorige sectie zijn beschreven. 
...
Het concept van minimaal bevoegdheden is een basisbeveiligings best practice die ook van toepassing is op containers. Wanneer een beveiligingsprobleem wordt misbruikt, krijgt de aanvaller over het algemeen toegang en bevoegdheden die gelijk zijn aan die van de gecompromitteerde toepassing of het proces. Door ervoor te zorgen dat containers werken met de laagste bevoegdheden en toegang die nodig zijn om de taak te kunnen doen, vermindert u de blootstelling aan risico's. 
...
U kunt de potentiële aanvalsmogelijkheden ook minimaliseren door ongebruikte of onnodige processen of bevoegdheden uit de container-runtime te verwijderen. Bevoorrechte containers worden uitgevoerd als hoofdmap. Als een kwaadwillende gebruiker of workload een escape-bestand maakt in een geprivilegieerde container, wordt de container vervolgens uitgevoerd als root op dat systeem.
...
Om containers in het ene subnet te beschermen tegen beveiligingsrisico's in een ander subnet, onderhoudt u netwerksegmentatie (of nanosegmentatie) of scheiding tussen de containers die worden uitgevoerd. Het onderhouden van netwerksegmentatie kan ook nodig zijn voor het gebruik van containers in branches die vereist zijn om te voldoen aan de nalevingsvereisten.  
...
Controleer uw resourceactiviteit, zoals bestanden, netwerken en andere resources die uw containers gebruiken. Het bewaken van resourceactiviteit en -verbruik is nuttig voor zowel prestatiebewaking als als veiligheidsmaatregel. 
...
### <a name="log-all-container-administrative-user-access-for-auditing"></a>Alle gebruikerstoegang met beheerderstoegang voor containers voor controlelogboek 

Behoudt een nauwkeurig audittrail van beheerderstoegang tot uw containerecosysteem, inclusief uw Kubernetes-cluster, containerregister en container-afbeeldingen. Deze logboeken kunnen nodig zijn voor controledoeleinden en zijn nuttig als forensisch bewijs na een beveiligingsincident. Azure-oplossingen zijn onder andere:
...
* Zie de [Azure-beveiligingsbasislijn Container Instances](security-baseline.md) voor uitgebreide aanbevelingen die u helpen de beveiligingsstatus van uw implementatie te verbeteren.
