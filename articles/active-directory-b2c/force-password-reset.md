...
Als beheerder kunt u het wacht woord van een gebruiker instellen op 90 dagen, met behulp van [MS Graph](microsoft-graph-operations.md). Na 90 dagen wordt de waarde van het kenmerk [forceChangePasswordNextSignIn](user-profile-attributes.md#password-profile-property) automatisch ingesteld op `true` . Zie [wachtwoord beleid kenmerk](user-profile-attributes.md#password-policy-attribute)voor meer informatie over het instellen van het wachtwoord verloop beleid van een gebruiker.
