...
ms.locfileid: "107767813"
---
# <a name="importing-azure-key-vault-certificates-faq"></a>Veelgestelde vragen Azure Key Vault importeren van certificaten
...
Voor een certificaatimportbewerking accepteert Azure Key Vault twee certificaatbestandsindelingen: PEM en PFX. Hoewel er PEM-bestanden zijn met alleen het openbare gedeelte, Key Vault alleen een PEM- of PFX-bestand met een persoonlijke sleutel vereist en accepteert. Zie Import [a certificate to Key Vault (Een certificaat importeren naar Key Vault) voor meer Key Vault.](./tutorial-import-certificate.md#import-a-certificate-to-key-vault)

### <a name="after-i-import-a-password-protected-certificate-to-key-vault-and-then-download-it-why-cant-i-see-the-password-thats-associated-with-it"></a>Waarom zie ik het bijbehorende wachtwoord niet nadat ik Key Vault met een wachtwoord heb geïmporteerd en vervolgens heb gedownload?
...
Wanneer u een certificaat importeert, moet u ervoor zorgen dat de sleutel in het bestand is opgenomen. Als u een persoonlijke sleutel afzonderlijk in een andere indeling hebt opgeslagen, moet u de sleutel combineren met het certificaat. Sommige certificeringsinstanties (CERTIFICERINGsinstanties) bieden certificaten in andere indelingen. Voordat u het certificaat importeert, moet u er daarom voor zorgen dat het de PEM- of PFX-bestandsindeling heeft en dat de sleutel gebruikmaakt van cryptografische versleuteling met elliptische of ecliptische curve (Arctische versleuteling) of RSA (Shamir–Adleman) of ECC-versleuteling (Elliptic-Curve). 

Zie certificaatvereisten [](./certificate-scenarios.md#formats-of-import-we-support) en certificaatsleutelvereisten voor [meer informatie.](../keys/about-keys.md)
...
Nee, het is niet mogelijk om certificaatbewerkingen uit te voeren met behulp van een AZURE RESOURCE MANAGER-sjabloon (ARM). Een aanbevolen tijdelijke oplossing is het gebruik van de importmethoden voor certificaten in de Azure-API, De Azure CLI of PowerShell. Als u een bestaand certificaat hebt, kunt u het als geheim importeren.

### <a name="when-i-import-a-certificate-via-the-azure-portal-i-get-a-something-went-wrong-error-how-can-i-investigate-further"></a>Wanneer ik een certificaat via de Azure Portal, krijg ik de fout 'Er is iets misgegaan'. Hoe kan ik dit verder onderzoeken?
     
Als u een meer beschrijvende fout wilt weergeven, importeert u het certificaatbestand met behulp [van de Azure CLI](/cli/azure/keyvault/certificate#az_keyvault_certificate_import) of [PowerShell.](/powershell/module/azurerm.keyvault/import-azurekeyvaultcertificate)
...
Zie About Azure Key Vault certificates (Informatie over Azure Key Vault [certificaten) voor meer informatie over het toegangsbeleid voor certificaten.](./about-certificates.md#certificate-access-control)
...
Elke certificaatnaam moet uniek zijn. Een certificaat met dezelfde naam heeft mogelijk de status 'soft-leted'. Volgens de samenstelling van een certificaat wordt er, wanneer er een nieuw certificaat wordt gemaakt, een adresseerbaar geheim met dezelfde naam gemaakt. Als er dus een andere sleutel of geheim in de sleutelkluis is met dezelfde naam als het geheim dat u voor uw certificaat probeert op te geven, mislukt het maken van het certificaat en moet u die sleutel [of](./about-certificates.md#composition-of-a-certificate)het geheim verwijderen of een andere naam voor uw certificaat gebruiken. 

Zie Get [Deleted Certificate operation voor meer informatie.](/rest/api/keyvault/getdeletedcertificate/getdeletedcertificate)
