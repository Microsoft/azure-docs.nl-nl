---
title: Windows Virtual Desktop thin client support-Azure
description: Een beknopt overzicht van ondersteuning voor thin clients voor virtuele Windows-Bureau bladen.
author: Heidilohr
ms.topic: conceptual
ms.date: 04/01/2021
ms.author: helohr
manager: femila
ms.openlocfilehash: 25f89b91aeb6e721c91356ef13b456455d4d3c98
ms.sourcegitcommit: 56b0c7923d67f96da21653b4bb37d943c36a81d6
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 04/06/2021
ms.locfileid: "106446057"
---
# <a name="thin-client-support"></a>Ondersteuning voor thin clients

U hebt toegang tot de virtuele bureau blad-resources van Windows vanaf uw thin client-apparaten met de [webclient](connect-web.md) of de volgende ondersteunde clients die door onze partners worden geboden. We werken samen met een aantal partners om ondersteunde Windows virtual desktop-clients op andere platforms in te scha kelen.

## <a name="connect-with-your-thin-client-device"></a>Verbinding maken met uw thin client-apparaat

De volgende partners hebben goedgekeurde Windows-clients met virtueel bureau blad.

|Partner|Documentatie voor partners|Partner ondersteuning|
|:------|:--------------------|:--------------|
|10ZiG |[documentatie voor 10ZiG-client](https://www.10zig.com/about/microsoft-windows-virtual-desktop)|[10ZiG-ondersteuning](https://www.10zig.com/resources/support_faq)|
|Dell |[Documentatie voor Dell-clients](https://www.delltechnologies.com/en-us/collaterals/unauth/data-sheets/products/thin-clients/dell-thinos-9-for-microsoft-wvd.pdf)|[Ondersteuning voor Dell](https://www.dell.com/support)|
|HP |[Documentatie voor HP-clients](https://h20195.www2.hp.com/v2/GetDocument.aspx?docname=c07051097)|[HP-ondersteuning](https://support.hp.com/us-en/products/workstations-thin-clients)|
|IGEL |[Documentatie voor IGEL-client](https://www.igel.com/igel-solution-family/windows-virtual-desktop/)|[IGEL-ondersteuning](https://www.igel.com/support/)|
|NComputing |[Documentatie voor NComputing-client](https://www.ncomputing.com/microsoft)|[NComputing-ondersteuning](https://www.ncomputing.com/support/support-options)|
|Stratodesk |[Documentatie voor Stratodesk-client](https://www.stratodesk.com/kb/Microsoft_Windows_Virtual_Desktop_(WVD))|[Stratodesk-ondersteuning](https://www.stratodesk.com/support/)|

## <a name="next-steps"></a>Volgende stappen

Bekijk onze documentatie voor de volgende clients:

- [Windows Desktop-client](connect-windows-7-10.md)
- [Webclient](connect-web.md)
- [Android-client](connect-android.md)
- [macOS-client](connect-macos.md)
- [iOS-client](connect-ios.md)
