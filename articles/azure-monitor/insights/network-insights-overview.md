---
title: Netwerkinzichten
description: Een overzicht van Network Insights, dat een uitgebreide weer gave biedt van de status en metrische gegevens voor alle geïmplementeerde netwerk bronnen zonder enige configuratie.
ms.subservice: ''
ms.topic: conceptual
author: KumudD
ms.author: kumud
...
ms.locfileid: "96751764"
---
# <a name="network-insights"></a>Netwerkinzichten

> [!NOTE]
> *Netwerk inzichten* werd eerder aangeduid als *Azure monitor voor netwerken*. 

Network Insights biedt een uitgebreid overzicht van de [status](../../service-health/resource-health-checks-resource-types.md) en [metrische gegevens](../platform/metrics-supported.md) voor alle geïmplementeerde netwerk bronnen, zonder dat hiervoor configuratie is vereist. Het biedt ook toegang tot netwerk bewakings mogelijkheden zoals [verbindings monitor](../../network-watcher/connection-monitor-preview.md), [flow logboek registratie voor netwerk beveiligings groepen (nsg's)](../../network-watcher/network-watcher-nsg-flow-logging-overview.md)en [Traffic Analytics](../../network-watcher/traffic-analytics.md). En het biedt andere [Diagnostische](../../network-watcher/network-watcher-monitoring-overview.md#diagnostics) functies voor het netwerk.

Network Insights is gestructureerd rond de volgende belang rijke onderdelen van bewaking:
...
De **overzichts** pagina van Network Insights biedt een eenvoudige manier om de inventaris van uw netwerk bronnen te visualiseren, samen met resource status en waarschuwingen. Het is onderverdeeld in vier functionele gebieden: zoeken en filteren, resource status en metrische gegevens, waarschuwingen en afhankelijkheids weergave.
...
[![Scherm opname van de zoek resultaten van Network Insights.](media/network-insights-overview/search.png)](media/network-insights-overview/search.png#lightbox)
...
![Scherm opname van de status en metrische gegevens van resources in netwerk inzichten.](media/network-insights-overview/resource-health.png)

Als u de niet-beschik bare geen en VPN-verbindingen selecteert, ziet u een metrische weer gave: 

![Scherm opname van de weer gave metrische gegevens in netwerk inzichten.](media/network-insights-overview/metric-view.png)
...
![Sreenshot waarin Application Gateway weer gave in netwerk inzichten wordt weer gegeven.](media/network-insights-overview/application-gateway.png)
...
[![Scherm opname van de weer gave van afhankelijkheden in netwerk inzichten.](media/network-insights-overview/dependency-view.png)](media/network-insights-overview/dependency-view.png#lightbox)
...
![Scherm opname van het menu met afhankelijkheids weergaven in netwerk inzichten.](media/network-insights-overview/dependency-view-menu.png)
...
![Scherm afbeelding met een voor beeld van een zoek opdracht in netwerk inzichten.](media/network-insights-overview/search-example.png)
...
![Scherm opname van het tabblad connectiviteit in netwerk inzichten.](media/network-insights-overview/azure-monitor-for-networks-connectivity-tab.png)
...
[![Scherm opname van connectiviteits tests in netwerk inzichten.](media/network-insights-overview/azure-monitor-for-networks-connectivity-tests.png)](media/network-insights-overview/azure-monitor-for-networks-connectivity-tests.png#lightbox)

U kunt een wille keurige bron-of doel tegel selecteren om een metrische weer gave te openen:

[![Scherm opname van de verbindings gegevens in netwerk inzichten.](media/network-insights-overview/azure-monitor-for-networks-connectivity-metrics.png)](media/network-insights-overview/azure-monitor-for-networks-connectivity-metrics.png#lightbox)
...
[![Scherm opname van het tabblad verkeer in netwerk inzichten.](media/network-insights-overview/azure-monitor-for-networks-traffic-view.png)](media/network-insights-overview/azure-monitor-for-networks-traffic-view.png#lightbox)
...
[![Scherm opname van de weer gave van het verkeers gebied in netwerk inzichten.](media/network-insights-overview/azure-monitor-for-networks-traffic-region-view.png)](media/network-insights-overview/azure-monitor-for-networks-traffic-region-view.png#lightbox)
...
Deze sectie helpt u bij het vaststellen en oplossen van enkele veelvoorkomende problemen die kunnen optreden wanneer u netwerk inzichten gebruikt. 

### <a name="how-do-i-resolve-performance-problems-or-failures"></a>Kunt u prestatie problemen of fouten Hoe kan ik oplossen?

Zie voor meer informatie over het oplossen van problemen met netwerken die u aanduidt met netwerk inzichten, de documentatie voor probleem oplossing voor de storings bron. 
