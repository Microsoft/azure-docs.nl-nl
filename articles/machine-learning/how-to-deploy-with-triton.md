...
Een van de manieren om een model voor de deferentie te implementeren, is als een webservice. Bijvoorbeeld een implementatie naar Azure Kubernetes Service of Azure Container Instances. Standaard gebruikt Azure Machine Learning web-framework voor *algemene* doeleinden met één thread voor webservice-implementaties.

Triton is een framework dat is *geoptimaliseerd voor de deferentie*. Het biedt een beter gebruik van GPU's en een rendabeler deferentie. Aan de serverzijde worden binnenkomende aanvragen in batches uitgevoerd en worden deze batches ter deferie ingediend. Batching maakt beter gebruik van GPU-resources en is een belangrijk onderdeel van de prestaties van Triton.

> [!IMPORTANT]
> Triton gebruiken voor implementatie vanuit Azure Machine Learning is momenteel in __preview.__ Preview-functionaliteit wordt mogelijk niet gedekt door de klantondersteuning. Zie voor meer informatie de [aanvullende gebruiksvoorwaarden voor Microsoft Azure previews](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)

> [!TIP]
> De codefragmenten in dit document zijn bedoeld ter illustratie en bieden mogelijk geen volledige oplossing. Zie de [end-to-end-voorbeelden](https://aka.ms/triton-aml-sample)van Triton in Azure Machine Learning voor een Azure Machine Learning.
...
* Er [worden meerdere Gunicorn-werksters](https://gunicorn.org/) gestart om gelijktijdig binnenkomende aanvragen te verwerken.
* Deze werksters verwerken voorverwerking, het aanroepen van het model en naverwerking. 
* Clients gebruiken de __Azure ML-score-URI__. Bijvoorbeeld `https://myservice.azureml.net/score`.

:::image type="content" source="./media/how-to-deploy-with-triton/normal-deploy.png" alt-text="Normaal, niet-triton, diagram van implementatiearchitectuur":::
...
:::image type="content" source="./media/how-to-deploy-with-triton/triton-deploy.png" alt-text="De deferenceconfig-implementatie met alleen Triton en geen Python-middleware":::
...
* Triton verwerkt aanvragen in batches om het GPU-gebruik te maximaliseren.
* De client gebruikt de __Azure ML-scoring-URI om__ aanvragen te maken. Bijvoorbeeld `https://myservice.azureml.net/score`.
...
1. Uw model rechtstreeks met Triton bedienen.
...
Het modelconfiguratiebestand vertelt Triton hoeveel invoer kan worden verwacht en van welke dimensies deze invoer zal zijn. Zie Modelconfiguratie [in](https://aka.ms/nvidia-triton-docs) de NVIDIA-documentatie voor meer informatie over het maken van het configuratiebestand.
...
> 
> Zie Gegenereerde modelconfiguratie [in](https://aka.ms/nvidia-triton-docs) de NVIDIA-documentatie voor meer informatie over deze optie.

### <a name="use-the-correct-directory-structure"></a>De juiste mapstructuur gebruiken

Wanneer u een model registreert bij Azure Machine Learning, kunt u afzonderlijke bestanden of een mapstructuur registreren. Als u Triton wilt gebruiken, moet de modelregistratie zijn voor een mapstructuur die een map met de naam `triton` bevat. De algemene structuur van deze map is:
...
> [!IMPORTANT]
> Deze mapstructuur is een Triton-modelopslagplaats en is vereist voor uw model(s) om met Triton te kunnen werken. Zie [Triton Model Repositories](https://aka.ms/nvidia-triton-docs) in de NVIDIA-documentatie voor meer informatie.
...
Bij het registreren van het model in Azure Machine Learning, moet de waarde voor de parameter de naam zijn van de `--model-path  -p` bovenliggende map van de Triton.  
In het bovenstaande voorbeeld  `--model-path` is 'modellen'.

De waarde voor `--name  -n` parameter, â€ ̃ my_triton_modelâ€™ in het voorbeeld, is de modelnaam die bekend staat bij Azure Machine Learning-werkruimte. 
...
Als u een gpu-cluster Azure Kubernetes Service met de naam 'aks-gpu' hebt gemaakt via Azure Machine Learning, kunt u de volgende opdracht gebruiken om uw model te implementeren.
...
Neem code op zoals in het volgende voorbeeld in uw `score.py` bestand. Triton in Azure Machine Learning verwacht te worden aangepakt op localhost, poort 8000. In dit geval maakt localhost voor deze implementatie gebruik van de Docker-installatier, niet van een poort op uw lokale computer:

> [!TIP]
> Het pip-pakket is opgenomen in de gecureerde omgeving, dus u hoeft het niet op te geven `tritonhttpclient` `AzureML-Triton` als pip-afhankelijkheid.
...
In het volgende voorbeeld wordt gedemonstreerd hoe u de metagegevens voor het model dynamisch kunt aanvragen:

> [!TIP]
> U kunt dynamisch de metagegevens aanvragen van modellen die met Triton zijn geladen met behulp van de `.get_model_metadata` methode van de Triton-client. Zie het [voorbeeldnote notebook](https://aka.ms/triton-aml-sample) voor een voorbeeld van het gebruik ervan.
...
### <a name="redeploy-with-an-inference-configuration"></a>Opnieuw wordenploy met een deference-configuratie
...
Nadat de implementatie is voltooid, wordt de scoring-URI weergegeven. Voor deze lokale implementatie is dit `http://localhost:6789/score` . Als u in de cloud implementeert, kunt u de CLI-opdracht [az ml service show](/cli/azure/ml/service#az_ml_service_show) gebruiken om de scoring-URI op te halen.
...
### <a name="setting-the-number-of-workers"></a>Het aantal werkpersoneel instellen

Als u het aantal werkrol in uw implementatie wilt instellen, stelt u de omgevingsvariabele `WORKER_COUNT` in. Als u een [omgevingsobject hebt](/python/api/azureml-core/azureml.core.environment.environment) met de naam , kunt u het `env` volgende doen:
...
* [Problemen met een mislukte implementatie oplossen,](how-to-troubleshoot-deployment.md)meer informatie over het oplossen van veelvoorkomende fouten die kunnen optreden bij het implementeren van een model.

* Als implementatielogboeken laten zien dat **TritonServer** niet kan worden starten, raadpleegt u de documentatie van [Nvidiaâ€™s open source.](https://github.com/triton-inference-server/server)
...
* Lees de [documentatie voor Triton Inference Server](https://aka.ms/nvidia-triton-docs)
