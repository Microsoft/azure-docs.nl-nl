...
|:--- |:--- |:--- |
| type | De `type` eigenschap moet worden ingesteld op `AzureBlobStorage` (aanbevolen) of `AzureStorage` (Zie de volgende opmerkingen). | Ja |
| connectionString | Geef de gegevens op die nodig zijn om verbinding te maken met de opslag voor de `connectionString` eigenschap. <br/> U kunt ook de account sleutel in Azure Key Vault plaatsen en de `accountKey` configuratie uit de Connection String halen. Zie voor meer informatie de volgende voor beelden en de [referenties van de winkel in azure Key Vault](store-credentials-in-key-vault.md) artikel. | Yes |
...
>[!NOTE]
>Als u de `AzureStorage` gekoppelde service van het type gebruikt, wordt deze nog steeds ondersteund. We raden u echter aan het nieuwe `AzureBlobStorage` gekoppelde service type verder te gebruiken.
...
| Eigenschap | Beschrijving | Vereist |
|:--- |:--- |:--- |
| type | De `type` eigenschap moet worden ingesteld op `AzureBlobStorage` (aanbevolen) of `AzureStorage` (Zie de volgende opmerking). | Ja |
...
>- Als u toegang hebt tot de Blob-opslag via een persoonlijk eind punt met behulp van de gegevens stroom, moet u er rekening mee houden wanneer Service-Principal-verificatie wordt gebruikt gegevens stroom verbinding maakt met het ADLS Gen2 eind punt in plaats van Zorg ervoor dat u het bijbehorende persoonlijke eind punt in ADF maakt om toegang in te scha kelen.
...
> - Als u de Blob-opslag via een persoonlijk eind punt opent met behulp van de gegevens stroom, Let er dan op dat de beheerde identiteits verificatie wordt gebruikt en dat de gegevens stroom verbinding maakt met het ADLS Gen2-eind punt in plaats van Zorg ervoor dat u het bijbehorende persoonlijke eind punt in ADF maakt om toegang in te scha kelen.
...
| OPTIE 2: BLOB-voor voegsel<br>-voor voegsel | Voor voegsel voor de BLOB-naam onder de opgegeven container geconfigureerd in een gegevensset om bron-blobs te filteren. Blobs waarvan de naam begint met `container_in_dataset/this_prefix` zijn geselecteerd. Er wordt gebruikgemaakt van het filter aan de service zijde voor Blob Storage, dat betere prestaties levert dan een filter voor joker tekens.<br><br>Als u een voor voegsel gebruikt en wilt kopiëren naar een op een bestand gebaseerde Sink met behoud van de hiërarchie, noteert u het pad na de laatste "/" in het voor voegsel blijven behouden. U hebt bijvoorbeeld  `container/folder/subfolder/file.txt` de bron en het voor voegsel geconfigureerd als `folder/sub` . vervolgens wordt het pad naar het bestand behouden `subfolder/file.txt` . | No                                                          |
...
> [!NOTE]
> De `$logs` container, die automatisch wordt gemaakt wanneer Opslaganalyse is ingeschakeld voor een opslag account, wordt niet weer gegeven wanneer een container vermelding wordt uitgevoerd via de Data Factory-gebruikers interface. Het bestandspad moet rechtstreeks worden ingesteld voor Data Factory om bestanden uit de container te gebruiken `$logs` .
...
| Eigenschap                 | Beschrijving                                                  | Vereist |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | De `type` eigenschap onder `storeSettings` moet worden ingesteld op `AzureBlobStorageWriteSettings` . | Yes      |
...
**Optie Bestands naam:** Hiermee wordt bepaald hoe de doel bestanden in de doelmap worden genoemd. De opties voor de bestands naam zijn:
   ...
   - **Patroon**: Voer een patroon in voor het inventariseren van de uitvoer bestanden per partitie. Bijvoorbeeld `loans[n].csv` maken `loans1.csv` , `loans2.csv` , enzovoort.
...
| Eigenschap | Beschrijving | Vereist |
|:--- |:--- |:--- |
| type | De `type` eigenschap van de DataSet moet worden ingesteld op `AzureBlob` . | Yes |
...
| fileName | De naam of het Joker teken filter voor de blobs onder de opgegeven `folderPath` waarde. Als u geen waarde opgeeft voor deze eigenschap, wijst de gegevensset naar alle blobs in de map. <br/><br/>Voor het filter zijn toegestane joker tekens: `*` (komt overeen met nul of meer tekens) en `?` (komt overeen met nul of één teken).<br/>-Voor beeld 1: `"fileName": "*.csv"`<br/>-Voor beeld 2: `"fileName": "???20180427.txt"`<br/>Gebruik `^` dit om te escapen als uw bestands naam een Joker teken of escape tekens bevat in.<br/><br/>Als `fileName` niet wordt opgegeven voor een uitvoer gegevensset en `preserveHierarchy` niet is opgegeven in de gebeurtenisopvanger, genereert de Kopieer activiteit automatisch de naam van de blob met het volgende patroon: "*gegevens. [ Run ID-GUID van activiteit]. [GUID if FlattenHierarchy]. [indeling indien geconfigureerd]. [compressie indien geconfigureerd]*". Bijvoorbeeld: "Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt. gz". <br/><br/>Als u kopieert vanuit een tabellaire bron met behulp van een tabel naam in plaats van een query, is het naam patroon `[table name].[format].[compression if configured]` . Bijvoorbeeld: "MyTable.csv". | No |
| modifiedDatetimeStart | Bestanden worden gefilterd op basis van het kenmerk: laatst gewijzigd. De bestanden worden geselecteerd als het tijdstip van de laatste wijziging binnen het tijds bereik ligt tussen `modifiedDatetimeStart` en `modifiedDatetimeEnd` . De tijd wordt toegepast op de UTC-tijd zone in de notatie "2018-12-01T05:00:00Z". <br/><br/> Houd er rekening mee dat het inschakelen van deze instelling van invloed is op de algehele prestaties van de verplaatsing van gegevens wanneer u grote hoeveel heden bestanden wilt filteren. <br/><br/> De eigenschappen kunnen zijn `NULL` , wat betekent dat er geen bestands kenmerk filter op de gegevensset wordt toegepast.  Wanneer `modifiedDatetimeStart` een datum/tijd-waarde heeft, maar wel `modifiedDatetimeEnd` `NULL` , de bestanden waarvan het kenmerk laatst gewijzigd groter is dan of gelijk is aan de datum/tijd-waarde, worden geselecteerd.  Als `modifiedDatetimeEnd` de waarde van een datum/tijd is `modifiedDatetimeStart` `NULL` , maar de bestanden waarvan het kenmerk laatst gewijzigd is, kleiner is dan de datum/tijd-waarde, wordt geselecteerd.| No |
...
| Eigenschap | Beschrijving | Vereist |
|:--- |:--- |:--- |
| type | De `type` eigenschap van de bron van de Kopieer activiteit moet worden ingesteld op `BlobSource` . | Yes |
| recursieve | Geeft aan of de gegevens recursief worden gelezen uit de submappen of alleen vanuit de opgegeven map. Houd er rekening mee dat wanneer `recursive` is ingesteld op `true` en de Sink een archief op basis van bestanden is, een lege map of submap niet wordt gekopieerd of gemaakt bij de sink.<br/>Toegestane waarden zijn `true` (standaard) en `false` . | No |
| maxConcurrentConnections |De bovengrens van gelijktijdige verbindingen die tot het gegevens archief zijn gemaakt tijdens de uitvoering van de activiteit. Geef alleen een waarde op als u gelijktijdige verbindingen wilt beperken.| No |
...
| Eigenschap | Beschrijving | Vereist |
|:--- |:--- |:--- |
| type | De `type` eigenschap van de Sink voor kopieer activiteiten moet worden ingesteld op `BlobSink` . | Yes |
| copyBehavior | Hiermee wordt het Kopieer gedrag gedefinieerd wanneer de bron bestanden van een gegevens archief op basis van een bestand zijn.<br/><br/>Toegestane waarden zijn:<br/><b>-PreserveHierarchy (standaard instelling)</b>: behoudt de bestands hiërarchie in de doelmap. Het relatieve pad van het bron bestand naar de bronmap is identiek aan het relatieve pad van het doel bestand naar de doelmap.<br/><b>-FlattenHierarchy</b>: alle bestanden in de bronmap bevinden zich in het eerste niveau van de doelmap. De doel bestanden hebben automatisch gegenereerde namen. <br/><b>-MergeFiles</b>: alle bestanden van de bronmap worden samengevoegd met één bestand. Als de naam van het bestand of de blob is opgegeven, is de naam van het samengevoegde bestand de opgegeven naam. Anders is het een automatisch gegenereerde bestands naam. | No |
| maxConcurrentConnections |De bovengrens van gelijktijdige verbindingen die tot het gegevens archief zijn gemaakt tijdens de uitvoering van de activiteit. Geef alleen een waarde op als u gelijktijdige verbindingen wilt beperken.| No |
