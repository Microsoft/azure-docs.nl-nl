...
1. Haal het IP-adres van uw VM op met [az vm show](/cli/azure/vm#az_vm_show) om verbinding te maken met uw VM:
...
>[!NOTE]
> Kijk [hier](backup-azure-restore-files-from-vm.md#step-2-ensure-the-machine-meets-the-requirements-before-executing-the-script) om te zien of u het script kunt uitvoeren op uw virtuele machine voordat u doorgaat.
...
3. Voer het script uit om het herstelpunt te koppelen. Geef de naam van uw eigen script op:
    ...
    Terwijl het script wordt uitgevoerd, wordt u gevraagd om een wachtwoord op te geven voor toegang tot het herstelpunt. Geef het wachtwoord op dat wordt weergegeven in de vorige opdracht [az backup restore files mount-rp](/cli/azure/backup/restore/files#az_backup_restore_files_mount_rp), waarmee het herstelscript werd gegenereerd.
...
7. Ontkoppel het herstelpunt van uw VM met [az backup restore files unmount-rp](/cli/azure/backup/restore/files#az_backup_restore_files_unmount_rp). In het volgende voorbeeld wordt het herstelpunt van de VM met de naam *myVM* in *myRecoveryServicesVault* ontkoppeld.
