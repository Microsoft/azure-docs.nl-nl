...
## <a name="the-defender-for-iot-platform"></a>Het Defender voor IoT-platform
...
Organisaties gebruiken Programmeer bare logic controllers (PLCs) voor het beheren van fysieke processen, zoals robot armen in fabrieken, het centrifugeren van turbines in Wind-Farms en centrifuges in kern centrales.
...
U kunt beleid voor blok keren automatisch instellen in de Forti Gate firewall op basis van Defender voor IoT-waarschuwingen.
...
1. Sla de API-sleutel op.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="Beschrijving van mobiele telefoon genereert automatisch nieuwe API-sleutel":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Verdacht verkeer blok keren met de Forti Gate-firewall

1. Selecteer **door sturen** in het linkerdeel venster.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="De optie voor het doorstuur venster in een sensor.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Selecteer **regels voor door sturen maken** en regel parameters definiëren.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Scherm opname van het venster doorstuur regel maken":::
    ...
    | **Protocollen** | Standaard zijn alle protocollen geselecteerd.<br><br>Als u een specifiek protocol wilt selecteren, selecteert u **specifiek** en selecteert u het protocol waarvoor deze regel wordt toegepast. |
    | **Zoekprogramma's** | Standaard zijn alle beveiligings engines betrokken.<br><br>Als u een specifieke beveiligings Engine wilt selecteren waarvoor deze regel wordt toegepast, selecteert u **specifiek** en selecteert u de engine. |
    | **Systeem meldingen** | De sensor *online-en offline* status door sturen. Deze optie is alleen beschikbaar als u bent aangemeld bij de on-premises beheer console. |

1. Als u wilt instrueren dat Defender voor IoT firewall regels naar Forti Gate verzendt, selecteert u **actie** en selecteert **u vervolgens verzenden naar Forti Gate**.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Maak een doorstuur regel venster en selecteer verzenden naar Forti Gate":::

1. De Forti Gate-doorstuur regel configureren:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Het venster voor het maken van een doorstuur regel configureren":::

1. Stel in het deel venster **acties** de volgende para meters in:

    | Parameter | Beschrijving |
    |--|--|
    | Host | Het IP-adres type van de Forti Gate-server. |
    | Poort | Het poort type van de Forti Gate-server. |
    | Gebruikersnaam | Het Forti Gate-server gebruikersnaam type. |
    | API-sleutel | Voer de API-sleutel in die u hebt gemaakt in Forti Gate. |
    | Inkomende interface| Definiëren hoe blok kering wordt uitgevoerd:<br /><br />Op **IP-adres**: maakt beleid voor blok keren altijd op panorama op basis van IP-adres.<br /><br />Op **FQDN of IP-adres**: maakt blokkerings beleid op panorama op basis van FQDN, indien aanwezig, anders IP-adres.| 
    | Uitgaande interface |Stel het e-mail adres voor de e-mail melding voor het beleid in. <br /><br /> **Opmerking**: Zorg ervoor dat u een e-mail server hebt geconfigureerd in XSense. Als er geen e-mail adres wordt ingevoerd, verzendt XSense geen e-mail melding.| 
    |Configureren| Stel de volgende opties in om het blok keren van de verdachte bronnen door de Forti Gate-firewall toe te staan: <br /><br />**Ongeldige functie codes blok keren**: schendingen van het protocol-ongeldige veld waarde is strijdig met de specificatie van het ICS-protocol (mogelijke aanvallen)<br /><br />Niet **-geautoriseerde PLC Programming/firmware-updates blok keren**: niet-geautoriseerde PLC-wijzigingen<br /><br />**Niet-gemachtigde PLC blok keren**: PLC stoppen (downtime)<br /><br />**Problemen met betrekking tot malware blok keren**: blok keren van de industriële malware-pogingen (Triton, NotPetya, enz.). U kunt de optie voor **automatisch blok keren** selecteren. In dat geval wordt de blok kering automatisch en onmiddellijk uitgevoerd.<br /><br />Niet- *geautoriseerd scannen blok keren*: niet-geautoriseerd scannen (mogelijke Reconnaissance)
