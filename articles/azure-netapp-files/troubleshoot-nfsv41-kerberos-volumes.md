...
|`Hostname lookup failed`   | U moet een zone voor reverse lookup maken op de DNS-server en vervolgens een PTR-record van de AD-hostcomputer toevoegen in de zone voor reverse lookup. <br> Stel dat het IP-adres van de AD-machine is `10.1.1.4` , de hostnaam van de AD-machine (zoals gevonden met de opdracht hostname) `AD1` en de domein naam is `myDomain.com` . De PTR-record die is toegevoegd aan de zone voor reverse lookup moet zijn `10.1.1.4 -> AD1.myDomain.com` . |
...
* [NFSv 4.1 Kerberos-versleuteling voor Azure NetApp Files configureren](configure-kerberos-encryption.md)
