...
### <a name="azure-digital-twins-dtdl-implementation-specifics"></a>Azure Digital Twins specifieke DTDL-implementaties

Niet alle services die gebruikmaken van DTDL implementeren exact dezelfde functies van DTDL. IoT-Plug en Play gebruikt bijvoorbeeld niet de DTDL-functies die voor grafieken zijn, terwijl Azure Digital Twins momenteel geen DTDL-opdrachten implementeert. 
...
* **Eigenschap:** eigenschappen zijn gegevensvelden die de status van een entiteit vertegenwoordigen (zoals de eigenschappen in veel objectgeoriënteerde programmeertalen). Eigenschappen hebben back-opslag en kunnen op elk moment worden gelezen.
...
* **Onderdeel:** met onderdelen kunt u uw modelinterface zo nodig bouwen als een assembly van andere interfaces. Een voorbeeld van een onderdeel is een *frontCamera-interface* (en een andere componentinterface *backCamera*) die worden gebruikt bij het definiëren van een model voor een *telefoon.* U moet eerst een interface voor *frontCamera* definiëren alsof het een eigen model is, waarna u hier naar kunt verwijzen bij het definiëren van *Telefoon.*

    Gebruik een onderdeel om iets te beschrijven dat een integraal onderdeel van uw oplossing is, maar geen afzonderlijke identiteit nodig heeft en die niet afzonderlijk in de tweelinggrafiek hoeft te worden gemaakt, verwijderd of opnieuw hoeft te worden rangschikken. Als u wilt dat entiteiten onafhankelijke bestaan in de tweelinggrafiek hebben, geeft u deze weer als afzonderlijke digitale tweelingen van verschillende modellen, verbonden door relaties *(zie* volgende opsommingsteken).
...
* **Relatie:** met relaties kunt u laten zien hoe een digitale tweeling kan worden betrokken bij andere digitale tweelingen. Relaties kunnen verschillende semantische betekenis hebben, zoals *contains* ('floor contains room'), *cools* ('hvac cools room'), *isBilledTo* ('floor is billed to user'), enzovoort. Met relaties kan de oplossing een grafiek van onderling gerelateerde entiteiten bieden. Relaties kunnen ook [eigen](#properties-of-relationships) eigenschappen hebben.
...
* **Telemetrie** lijkt meer op een stroom gebeurtenissen; Het is een set gegevensberichten met een korte levensduur. Als u het luisteren naar de gebeurtenis en acties die moeten worden ondernomen wanneer deze zich voor doen niet in stelt, is er op een later tijdstip geen trace van de gebeurtenis. U kunt er niet meer naar teruggaan en later lezen. 
...
Telemetrie en eigenschappen werken vaak samen om ingressiegegevens van apparaten te verwerken. Omdat alle ingressen naar Azure Digital Twins via API's [zijn,](how-to-use-apis-sdks.md)gebruikt u doorgaans uw ingress-functie om telemetrie- of eigenschapsgebeurtenissen van apparaten te lezen en stelt u een eigenschap in Azure Digital Twins als reactie. 
...
#### <a name="properties-of-relationships"></a>Eigenschappen van relaties

Met DTDL kunnen **relaties** ook hun eigen eigenschappen hebben. Wanneer u een relatie binnen een DTDL-model definieert, kan de relatie een eigen veld hebben waarin u aangepaste eigenschappen kunt definiëren om de `properties` relatiespecifieke status te beschrijven.

## <a name="model-inheritance"></a>Model overname

Soms wilt u een model misschien verder specialiseren. Het kan bijvoorbeeld handig zijn om een algemeen model *Room* en gespecialiseerde varianten ConferenceRoom en *ConferenceRoom* te *hebben.* Om specialisatie uit te drukken, ondersteunt DTDL overname: interfaces kunnen overnemen van een of meer andere interfaces. 

In het volgende voorbeeld wordt het *Planet-model* uit het eerdere DTDL-voorbeeld opnieuw voorgesteld als een subtype van een groter *Model Van Nubody.* Het bovenliggende model wordt eerst gedefinieerd en vervolgens bouwt het onderliggende model daarop voort met behulp van het veld `extends` .

:::code language="json" source="~/digital-twins-docs-samples/models/CelestialBody-Planet-Crater.json":::

In dit voorbeeld draagt *DeBody* een naam, een massa en een temperatuur bij aan *De planeet*. De sectie is een interfacenaam of een matrix met interfacenamen (zodat de uitbreidbare interface kan worden overgenomen van meerdere `extends` bovenliggende modellen, indien gewenst).

Zodra overname is toegepast, worden met de uitbreidingsinterface alle eigenschappen uit de gehele overnameketen weer geven.

De uitbreidingsinterface kan geen van de definities van de bovenliggende interfaces wijzigen; het kan er alleen aan toevoegen. Het kan ook een mogelijkheid die al is gedefinieerd in een van de bovenliggende interfaces niet opnieuw definiëren (zelfs niet als de mogelijkheden zijn gedefinieerd als dezelfde). Als een bovenliggende interface bijvoorbeeld een eigenschaps massa definieert, kan de uitbreidingsinterface geen declaratie van massa bevatten, zelfs niet als het `double` ook een   `double` is.

## <a name="model-code"></a>Modelcode

Dubbeltypemodellen kunnen in elke teksteditor worden geschreven. De DTDL-taal volgt de JSON-syntaxis, dus u moet modellen opslaan met de extensie *.json.* Door de JSON-extensie te gebruiken, kunnen veel teksteditors eenvoudige syntaxiscontrole en markeringen bieden voor uw DTDL-documenten. Er is ook een [DTDL-extensie](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-dtdl) beschikbaar [voor Visual Studio Code](https://code.visualstudio.com/).
...
*Telemetrievelden* bieden ook ondersteuning voor `Array` .
...
Houd er rekening mee dat planeten ook kunnen communiceren **met manen** die hun satellieten zijn en **ook nog eens kunnen bevatten.** In het onderstaande voorbeeld drukt het model verbindingen met deze andere entiteiten uit door te verwijzen naar twee `Planet` externe modellen: `Moon` en `Crater` . Deze modellen worden ook gedefinieerd in de onderstaande voorbeeldcode, maar worden zeer eenvoudig gehouden, zodat dit niet af doet aan het primaire `Planet` voorbeeld.
...
Bij het ontwerpen van modellen die de entiteiten in uw omgeving weerspiegelen, kan het handig zijn om vooruit te kijken en rekening te houden met de [query-implicaties](concepts-query-language.md) van uw ontwerp. Mogelijk wilt u eigenschappen zo ontwerpen dat grote resultatensets worden voorkomen door het doorkruisen van grafen. U kunt ook relaties modelleren die in één query worden beantwoord als relaties met één niveau.
...
Er zijn verschillende voorbeelden beschikbaar om het nog eenvoudiger te maken om met modellen en ontologieën om te gaan. Ze bevinden zich in deze opslagplaats: [Tools for Digital Twins Definition Language (DTDL)](https://github.com/Azure/opendigitaltwins-tools).
...
* Meer informatie over hoe modellen worden gebruikt voor het maken van digitale tweelingen: [ *Concepten: Digitale tweelingen en de tweelinggrafiek*](concepts-twins-graph.md)
