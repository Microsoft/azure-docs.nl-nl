...
- **Via https verbinding maken met de Power shell-interface** : door deze release te starten, kunt u een externe Power shell-sessie niet meer openen in een apparaat via *http*. *Https* wordt standaard gebruikt voor alle sessies. Zie [verbinding maken met de Power shell-interface](azure-stack-edge-gpu-connect-powershell-interface.md#connect-to-the-powershell-interface) van uw apparaat voor meer informatie.

- **Verbeteringen voor Compute** -verschillende uitbrei dingen en verbeteringen zijn aangebracht, waaronder die voor:

    - **Algehele kwaliteit van computer platform**. Deze release bevat fout oplossingen voor het verbeteren van de algehele kwaliteit van het computer platform. Zie de [problemen die zijn opgelost in 2103-release](#issues-fixed-in-2103-release). 
    - **Compute platform-onderdelen**. Er zijn beveiligings updates toegepast op het berekenen van de VM-installatie kopie. IoT Edge en Azure Arc voor Kubernetes-versies zijn ook bijgewerkt.
    - **Diagnostische gegevens**. Er wordt een nieuwe API uitgebracht om de resource-en netwerk voorwaarden te controleren. U kunt verbinding maken met de Power shell-interface van het apparaat en de `Test-HcsKubernetesStatus` opdracht gebruiken om de netwerk voorbereiding van het apparaat te controleren.
    - **Logboek verzameling** die ertoe kan leiden dat de fout opsporing wordt verbeterd. 
    - **Infra structuur voor waarschuwingen** waarmee u IP-adres conflicten voor COMPUTe IP-adressen kunt detecteren. 
    - **Mix-werk belasting** van Kubernetes en lokale Azure Resource Manager. 

- **Proactieve logboek registratie standaard** -start deze release, proactieve logboek verzameling is standaard ingeschakeld op uw apparaat. Met deze functie kunnen micro soft logboeken proactief verzamelen op basis van de status indicatoren van het systeem om problemen met apparaten efficiënt op te lossen. Zie [proactieve logboek verzameling op uw apparaat](azure-stack-edge-gpu-proactive-log-collection.md)voor meer informatie.

## <a name="issues-fixed-in-2103-release"></a>Problemen opgelost in 2103-release

De volgende tabel bevat de problemen die zijn vermeld in eerdere releases en die in de huidige release zijn opgelost.

| Nee. | Functie | Probleem | 
| --- | --- | --- |
|**1.**|Kubernetes |Het Edge-container register werkt niet als webproxy is ingeschakeld.|
|**2.**|Kubernetes |Het Edge-container register werkt niet met IoT Edge-modules.| 

## <a name="known-issues-in-2103-release"></a>Bekende problemen in de 2103-release

De volgende tabel bevat een samen vatting van bekende problemen in de 2103-release.

| Nee. | Functie | Probleem | Tijdelijke oplossing/opmerkingen |
| --- | --- | --- | --- |
|**1.**|Preview-functies |Voor deze release zijn de volgende functies: lokale Azure Resource Manager, Vm's, Cloud beheer van Vm's, Kubernetes-Cloud beheer, Azure-Arc-Kubernetes, VPN voor Azure Stack Edge Pro R en Azure Stack Edge mini R, service voor meerdere processen (MPS) voor Azure Stack Edge Pro GPU-zijn allemaal beschikbaar als preview-versie.  |Deze functies zijn algemeen beschikbaar in latere versies. |
|**2.**|GPU-Vm's |Vóór deze release werd de GPU VM-levens cyclus niet beheerd in de update stroom. Daarom worden GPU-Vm's tijdens het bijwerken naar 2103-release niet automatisch gestopt tijdens de update. U moet de GPU-Vm's hand matig stoppen met een `stop-stayProvisioned` vlag voordat u uw apparaat bijwerkt. Zie [de virtuele machine onderbreken of afsluiten](azure-stack-edge-gpu-deploy-virtual-machine-powershell.md#suspend-or-shut-down-the-vm)voor meer informatie.<br> Alle GPU-Vm's die worden uitgevoerd vóór de update, worden na de update gestart. In deze gevallen worden de workloads die op de virtuele machines worden uitgevoerd, niet op de juiste wijze afgebroken. En de virtuele machines kunnen na de update mogelijk een onwenselijke status beëindigen. <br>Alle GPU-Vm's die worden gestopt via de `stop-stayProvisioned` vóór de update, worden automatisch gestart na de update. <br>Als u de GPU-Vm's stopt via de Azure Portal, moet u de virtuele machine hand matig starten na het bijwerken van het apparaat.| Als GPU-Vm's worden uitgevoerd met Kubernetes, stopt u de GPU-Vm's direct vóór de update. <br>Wanneer de GPU-Vm's worden gestopt, neemt Kubernetes de Gpu's over die oorspronkelijk door Vm's zijn gebruikt. <br>Hoe langer de GPU-Vm's de status gestopt hebben, des te hoger de kans dat Kubernetes de Gpu's overneemt. |
|**3.**|VM-extensie voor aangepast script |Er is een bekend probleem in de Windows-Vm's die zijn gemaakt in een eerdere versie en het apparaat is bijgewerkt naar 2103. <br> Als u een aangepaste script extensie aan deze virtuele machines toevoegt, wordt de Windows VM-gast agent (alleen versie 2.7.41491.901) vastgelopen in de update waardoor de extensie-implementatie een time-out heeft. | Dit probleem omzeilen: <ul><li> Maak verbinding met de Windows-VM met behulp van Remote Desktop Protocol (RDP). </li><li> Zorg ervoor dat de `waappagent.exe` wordt uitgevoerd op de computer: `Get-Process WaAppAgent` . </li><li> Als het `waappagent.exe` niet wordt uitgevoerd, start u de `rdagent` service opnieuw: `Get-Service RdAgent` \| `Restart-Service` . Wacht vijf minuten.</li><li> `waappagent.exe`Beëindig het proces terwijl het wordt uitgevoerd `WindowsAzureGuest.exe` . </li><li>Nadat u het proces hebt gestopt, wordt het proces opnieuw gestart met de nieuwere versie.</li><li>Controleer of de versie van de Windows VM-gast agent is 2.7.41491.971 met behulp van deze opdracht: `Get-Process WindowsAzureGuestAgent` \| `fl ProductVersion` .</li><li>[Aangepaste script extensie instellen op Windows VM](azure-stack-edge-gpu-deploy-virtual-machine-custom-script-extension.md). </li><ul> | 
|**4.**|Service voor meerdere processen (MPS) |Wanneer de software van het apparaat en het Kubernetes-cluster worden bijgewerkt, wordt de MPS-instelling niet bewaard voor de werk belastingen.   |[Schakel MPS opnieuw in](azure-stack-edge-gpu-connect-powershell-interface.md#connect-to-the-powershell-interface) en implementeer de werk belastingen die gebruikmaken van MPS opnieuw. |
